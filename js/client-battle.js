(function ($) {
	var BattleRoom = (this.BattleRoom = ConsoleRoom.extend(
		{
			...
			initialize: function (data) {
				...
				this.$el
					.addClass("ps-room-opaque")
					.html(
						'<div class="battle">Battle is here</div><div class="foehint"></div><div class="battle-log" aria-label="Battle Log" role="complementary"></div><div class="battle-log-add">Connecting...</div><ul class="battle-userlist userlist userlist-minimized"></ul><div class="battle-controls" role="complementary" aria-label="Battle Controls"></div><button class="battle-chat-toggle button" name="showChat"><i class="fa fa-caret-left"></i> Chat</button>'
					);
				...
				this.$options = this.battle.scene.$options.html(
					'<div style="padding-top: 3px; padding-right: 3px; text-align: right"><button class="icon button" name="openBattleOptions" title="Options">Battle Options</button></div>'
				);
			},
			events: {
			...
			battleEnded: false,
			join: function () {
				console.debug("sending /join", this.id);
				app.send("/join " + this.id);
			},
			showChat: function () {
				this.$(".battle-chat-toggle")
					.attr("name", "hideChat")
					.html('Battle <i class="fa fa-caret-right"></i>');
				this.$el.addClass("showing-chat");
			},
			hideChat: function () {
				this.$(".battle-chat-toggle")
					.attr("name", "showChat")
					.html('<i class="fa fa-caret-left"></i> Chat');
			...
			requestLeave: function (e) {
				if (
					...
					!this.battleEnded &&
					!this.expired &&
					!this.battle.forfeitPending
				) {
					app.addPopup(ForfeitPopup, {
						room: this,
						sourceEl: e && e.currentTarget,
						gameType: "battle",
					});
					return false;
				}
			...
			add: function (data) {
				...
				if (data.substr(0, 9) === "|request|") {
					...
					if (/[0-9]/.test(data.charAt(0)) && data.charAt(1) === "|") {
						...
						//   |sentchoice|CHOICE
						if (
							data.slice(nlIndex + 1, nlIndex + 13) === "|sentchoice|"
						) {
				...
				for (var i = 0; i < log.length; i++) {
					...
					if (logLine.substr(0, 10) === "|callback|") {
						...
						var pokemon = isNaN(Number(args[1]))
							? this.battle.getPokemon(args[1])
							: this.battle.nearSide.active[args[1]];
						var requestData =
							this.request.active[pokemon ? pokemon.slot : 0];
						this.choice = undefined;
						switch (args[0]) {
							case "trapped":
			...
			updateControls: function () {
				...
				var switchSidesButton =
					'<p><button class="button" name="switchSides"><i class="fa fa-random"></i> Switch sides</button></p>';
				this.controlsShown = false;
				...
				if (this.battle.ended) {
					var replayDownloadButton =
						'<span style="float:right;"><a href="//' +
						Config.routes.replays +
						'/" class="button replayDownloadButton"><i class="fa fa-download"></i> Download replay</a><br /><br /><button class="button" name="saveReplay"><i class="fa fa-upload"></i> Upload and share replay</button></span>';
					...
					!this.battle.nearSide.name ||
					!this.battle.farSide.name
				) {
					// empty battle
			...
			getTimerHTML: function (nextTick) {
				...
				return (
					'<button name="openTimer" class="button timerbutton' +
					timerTicking +
			...
			updateSwitchControls: function (type) {
				...
				if (type === "switchposition") {
					...
					var controls =
						'<div class="switchcontrols">' +
						'<div class="switchselect"><button name="selectSwitch">' +
						(isReviving ? "Revive" : "Switch") +
						"</button></div>" +
						'<div class="switchmenu">' +
						switchMenu +
						"</div>" +
			...
			updateTeamControls: function (type) {
				...
				var controls =
					...
					'<div class="switchselect"><button name="selectSwitch">' +
					(this.choice.done
						? "" + "Choose a Pokémon for slot " + (this.choice.done + 1)
						: "Choose Lead") +
					"</button></div>" +
					'<div class="switchmenu">' +
					switchMenu +
			...
			updateWaitControls: function () {
				...
				if (
					...
					if (this.battle.kickingInactive) {
						buf +=
							'<p><button class="button" name="setTimer" value="off">Stop timer</button> <small>&larr; Your opponent has disconnected. This will give them more time to reconnect.</small></p>';
					} else {
						buf +=
							'<p><button class="button" name="setTimer" value="on">Claim victory</button> <small>&larr; Your opponent has disconnected. Click this if they don\'t reconnect.</small></p>';
					}
				}
			...
			getPlayerChoicesHTML: function () {
				...
				if (this.choice.teamPreview) {
					...
					if (back.length)
						buf += back.join(", ") + " are in the back.<br />";
				} else if (
					this.choice.choices &&
					this.request &&
					...
					for (var i = 0; i < this.choice.choices.length; i++) {
						...
						switch (parts[0]) {
							case "move":
								var move;
								if (
									this.request.active[i].maxMoves &&
									!this.request.active[i].canDynamax
								) {
									// it's a max move
									move =
										this.request.active[i].maxMoves.maxMoves[
											parseInt(parts[1], 10) - 1
										].move;
								} else {
									// it's a normal move
									move =
										this.request.active[i].moves[
											parseInt(parts[1], 10) - 1
										].move;
								}
								var target = "";
								...
								if (parts.length > 2) {
									...
									if (targetPos === "zmove") {
										move =
											this.request.active[i].canZMove[
												parseInt(parts[1], 10) - 1
											].move;
										targetPos = parts[3];
									}
									...
									if (targetPos) {
										...
										if (targetActive[targetPos - 1]) {
											target +=
												targetActive[targetPos - 1].speciesForme;
										} else {
							...
							case "switch":
								buf +=
									"" +
									myPokemon[parts[1] - 1].speciesForme +
									" will switch in";
								if (myPokemon[i]) {
							...
							case "shift":
								buf +=
									myPokemon[i].speciesForme +
									" will shift position.<br />";
								break;
						}
					}
				}
				...
				if (!this.finalDecision && !this.battle.hardcoreMode) {
					buf +=
						'<p><small><em>Waiting for opponent...</em></small> <button class="button" name="undoChoice">Cancel</button></p>';
				}
				return buf;
			},
			...
			sendDecision: function (message) {
				if (!$.isArray(message))
					return this.send("/" + message + "|" + this.request.rqid);
				var buf = "/choose ";
			...
			receiveRequest: function (request, choiceText) {
				...
				if (
					!this.autoTimerActivated &&
					Storage.prefs("autotimer") &&
					!this.battle.ended
				) {
					this.setTimer("on");
			...
			notifyRequest: function () {
				...
				switch (this.request.requestType) {
					...
					case "team":
						this.notify(
							"Team preview!",
							"Choose your team order in your battle" + oName,
							"choice"
						);
						break;
				}
			...
			updateSide: function () {
				...
				for (var i = 0; i < sideData.pokemon.length; i++) {
					...
					if (this.request.active && this.request.active[i])
						pokemonData.canGmax =
							this.request.active[i].gigantamax || false;
					this.battle.parseDetails(
						pokemonData.ident.substr(4),
						pokemonData.ident,
						pokemonData.details,
						pokemonData
					);
					...
					pokemonData.getFormattedRange =
						Pokemon.prototype.getFormattedRange;
			...
			openBattleOptions: function () {
				app.addPopup(BattleOptionsPopup, {
					battle: this.battle,
					room: this,
				});
			},
			clickReplayDownloadButton: function (e) {
				var filename = (this.battle.tier || "Battle").replace(
					/[^A-Za-z0-9]/g,
					""
				);
			...
			chooseMove: function (pos, e) {
				...
				if (pos !== undefined) {
					...
					var isUltraBurst = !!(this.$("input[name=ultraburst]")[0] || "")
						.checked;
					var isDynamax = !!(this.$("input[name=dynamax]")[0] || "")
						.checked;
					var isTerastal = !!(this.$("input[name=terastallize]")[0] || "")
						.checked;
			...
			chooseMoveTarget: function (posString) {
				this.choice.choices[this.choice.choices.length - 1] +=
					" " + posString;
				this.chooseMove();
			},
			...
			chooseSwitch: function (pos) {
				...
				if (pos !== undefined) {
					// pos === undefined if called by chooseSwitchTarget()
				...
				if (
					this.choice.canSwitch >
					_.filter(this.choice.choices, function (choice) {
						return choice;
					}).length
				) {
			...
			chooseTeamPreview: function (pos) {
				...
				if (this.choice.count) {
					...
					this.choice.teamPreview[pos] =
						this.choice.teamPreview[this.choice.done];
					...
					if (
						this.choice.done <
						Math.min(this.choice.teamPreview.length, this.choice.count)
					) {
			...
			nextChoice: function () {
				...
				if (
					this.request.requestType === "switch" &&
					this.request.forceSwitch !== true
				) {
					while (
						choices.length < this.battle.pokemonControlled &&
						!this.request.forceSwitch[choices.length]
					) {
						choices.push("pass");
					}
					...
					var requestDetails =
						this.request && this.request.side
							? this.battle.myPokemon
							: [];
					while (
						choices.length < this.battle.pokemonControlled &&
						(!nearActive[choices.length] ||
							requestDetails[choices.length].commanding)
					) {
						choices.push("pass");
					}
			...
			endTurn: function () {
				...
				if (act === "team") {
					if (this.choice.teamPreview.length >= 10) {
						this.sendDecision(
							"team " + this.choice.teamPreview.join(",")
						);
					} else {
					...
					if (act === "switch") {
						...
						for (var i = 0; i < this.battle.pokemonControlled; i++) {
							if (!this.choice.choices[i])
								this.choice.choices[i] = "pass";
						}
					}

					if (
						this.choice.choices.length >=
						(this.choice.count ||
							this.battle.pokemonControlled ||
							this.request.active.length)
					) {
					...
					if (!this.finalDecision) {
						var lastChoice =
							this.choice.choices[this.choice.choices.length - 1];
						if (
							lastChoice.substr(0, 5) === "move " &&
							this.finalDecisionMove
						) {
							this.finalDecisionMove = true;
						} else if (
							lastChoice.substr(0, 7) === "switch" &&
							this.finalDecisionSwitch
						) {
			...
			selectSwitch: function () {
				...
				this.$controls
					.find(".controls")
					.attr("class", "controls switch-controls");
			},
			selectMove: function () {
				this.tooltips.hideTooltip();
				this.$controls
					.find(".controls")
					.attr("class", "controls move-controls");
			},
		},
		{
			readReplayFile: function (file) {
				...
				reader.onload = function (e) {
					...
					if (index1 < 0 && index2 < 0)
						return alert(
							"Unrecognized HTML file: Only replay files are supported."
						);
					if (index1 >= 0) {
	...
	var ForfeitPopup = (this.ForfeitPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (this.gameType === "help") {
				buf +=
					' Are you sure?</p><p><label><input type="checkbox" name="closeroom" checked /> Close room</label></p>';
				buf +=
					'<p><button type="submit"><strong>Close ticket</strong></button> ';
			} else if (this.gameType === "room") {
				buf +=
					' </p><p><button type="leaveRoom" name="leaveRoom"><strong>Close room</strong></button>';
			} else {
				buf +=
					' Are you sure?</p><p><label><input type="checkbox" name="closeroom" checked /> Close after forfeiting</label></p>';
				buf +=
					'<p><button type="submit"><strong>Forfeit</strong></button> ';
			}
			if (
				this.gameType === "battle" &&
				this.room.battle &&
				!this.room.battle.rated
			) {
				buf +=
					'<button type="button" name="replacePlayer">Replace player</button> ';
			}
			buf +=
				'<button type="button" name="close" class="autofocus">Cancel</button></p></form>';
			this.$el.html(buf);
		},
		replacePlayer: function (data) {
			...
			var self = this;
			app.addPopupPrompt(
				"Replacement player's username",
				"Replace player",
				function (target) {
					if (!target) return;
					var side =
						room.battle.mySide.id === room.battle.p1.id ? "p1" : "p2";
					room.leaveBattle();
	...
	var BattleOptionsPopup = (this.BattleOptionsPopup = Popup.extend({
		initialize: function (data) {
			...
			this.room = data.room;
			var rightPanelBattlesPossible =
				MainMenuRoom.prototype.bestWidth + BattleRoom.prototype.minWidth <
				$(window).width();
			var buf = "<p><strong>In this battle</strong></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="hardcoremode"' +
				(this.battle.hardcoreMode ? " checked" : "") +
				"/> Hardcore mode (hide info not shown in-game) (beta)</label></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="ignorespects"' +
				(this.battle.ignoreSpects ? " checked" : "") +
				"/> Ignore spectators</label></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="ignoreopp"' +
				(this.battle.ignoreOpponent ? " checked" : "") +
				"/> Ignore opponent</label></p>";
			buf += "<p><strong>All battles</strong></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="ignorenicks"' +
				(Dex.prefs("ignorenicks") ? " checked" : "") +
				" /> Ignore nicknames</label></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="allignorespects"' +
				(Dex.prefs("ignorespects") ? " checked" : "") +
				"/> Ignore spectators</label></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="allignoreopp"' +
				(Dex.prefs("ignoreopp") ? " checked" : "") +
				"/> Ignore opponent</label></p>";
			buf +=
				'<p><label class="optlabel"><input type="checkbox" name="autotimer"' +
				(Dex.prefs("autotimer") ? " checked" : "") +
				"/> Automatically start timer</label></p>";
			if (rightPanelBattlesPossible)
				buf +=
					'<p><label class="optlabel"><input type="checkbox" name="rightpanelbattles"' +
					(Dex.prefs("rightpanelbattles") ? " checked" : "") +
					" /> Open new battles on the right side</label></p>";
		...
		toggleIgnoreSpects: function (e) {
			...
			this.battle.add(
				"Spectators " +
					(this.battle.ignoreSpects ? "" : "no longer ") +
					"ignored."
			);
			var $messages = $(".battle-log")
				.find(".chat")
				.has("small")
		...
		toggleAllIgnoreSpects: function (e) {
			...
			if (ignoreSpects && !this.battle.ignoreSpects)
				this.$el.find("input[name=ignorespects]").click();
		},
		toggleIgnoreNicks: function (e) {
			...
			this.battle.add(
				"Nicknames " +
					(this.battle.ignoreNicks ? "" : "no longer ") +
					"ignored."
			);
		...
		toggleIgnoreOpponent: function (e) {
			...
			this.battle.add(
				"Opponent " +
					(this.battle.ignoreOpponent ? "" : "no longer ") +
					"ignored."
			);
			this.battle.resetToCurrentTurn();
		},
		toggleAllIgnoreOpponent: function (e) {
			...
			if (ignoreOpponent && !this.battle.ignoreOpponent)
				this.$el.find("input[name=ignoreopp]").click();
		},
	...
	var TimerPopup = (this.TimerPopup = Popup.extend({
		initialize: function (data) {
			...
			if (this.room.battle.kickingInactive) {
				this.$el.html(
					'<p><button name="timerOff"><strong>Stop timer</strong></button></p>'
				);
			} else {
				this.$el.html(
					'<p><button name="timerOn"><strong>Start timer</strong></button></p>'
				);
			}
		},
		timerOff: function () {
