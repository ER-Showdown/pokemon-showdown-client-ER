	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		focus: function () {
			if (this.curTeam) {
				...
				if (this.curTeam.format.includes("eliteredux")) {
					this.curTeam.dex = Dex.mod("gen8eliteredux");
				}
		...
		update: function () {
			...
			if (this.curTeam) {
				this.ignoreEVLimits =
					this.curTeam.gen < 3 ||
					((this.curTeam.format.includes("hackmons") ||
						this.curTeam.format.endsWith("bh")) &&
						this.curTeam.gen !== 6) ||
		...
		updateTeamInterface: function () {
			...
			if (this.exportMode) {
				if (this.curFolder) {
					buf =
						'<div class="pad"><button name="back"><i class="fa fa-chevron-left"></i> List</button></div>';
					buf +=
					...
					buf =
						'<div class="pad"><button name="back"><i class="fa fa-chevron-left"></i> List</button> <button name="saveBackup" class="savebutton"><i class="fa fa-floppy-o"></i> Save</button></div>';
					buf +=
			...
			if (Storage.teams)
				for (var i = -2; i < Storage.teams.length; i++) {
					...
					if (i === -2) {
						...
						if (!format) format = "gen8elitereduxou";
					}
					if (!format) continue;
		...
		updateTeamList: function (resetScroll) {
			...
			if (!this.curFolder) {
				...
				buf +=
					'<p><button class="button" name="greeting" value="Y"><i class="fa fa-smile-o"></i> Yes, my day was pretty good</button> <button class="button" name="greeting" value="N"><i class="fa fa-frown-o"></i> No, it wasn\'t great</button></p>';
				buf +=
			...
			if (!teams.length) {
				...
				for (var i = 0; i < teams.length + 1; i++) {
					...
					if (team.format) {
						if (
							team.format.includes("eliteredux") &&
							window.BattleFormats
						) {
							formatText = `[${
								window.BattleFormats[team.format.substring(4)].name
							}] `;
						} else {
			...
			if (atLeastOne) {
				buf +=
					'<p><button name="new" value="team" class="button"><i class="fa fa-plus-circle"></i> ' +
					newTeamButtonText +
					'</button> <button name="new" value="box" class="button"><i class="fa fa-archive"></i> New Box</button></p>';
			}
			...
			if (window.nodewebkit) {
				...
				buf +=
					'<p><strong>Clearing your cookies (specifically, <code>localStorage</code>) will delete your teams.</strong> <span class="storage-warning">Browsers sometimes randomly clear cookies - you should back up your teams or use the desktop client if you want to make sure you don\'t lose them.</span></p>';
				buf +=
		...
		createTeam: function (orig, isBox) {
			...
			if (orig) {
				...
				var format = this.curFolder || "gen8elitereduxou";
				var folder = "";
				if (format && format.charAt(format.length - 1) === "/") {
					folder = format.slice(0, -1);
					format = "gen8elitereduxou";
				}
				newTeam = {
		...
		updateTeamView: function () {
			...
			if (this.exportMode) {
				buf =
					'<div class="pad"><button name="back"><i class="fa fa-chevron-left"></i> List</button> <input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' +
					BattleLog.escapeHTML(this.curTeam.name) +
					'" /> <button name="saveImport"><i class="fa fa-upload"></i> Import/Export</button> <button name="saveImport" class="savebutton"><i class="fa fa-floppy-o"></i> Save</button></div>';
				buf +=
					'<div class="teamedit"><textarea class="textbox" rows="17">' +
				...
				buf =
					'<div class="pad"><button name="back"><i class="fa fa-chevron-left"></i> List</button> ';
				buf +=
					'<input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' +
					BattleLog.escapeHTML(this.curTeam.name) +
					'" /> ';
				buf +=
					'<button name="import"><i class="fa fa-upload"></i> Import/Export</button> ';
				...
				if (exports.BattleFormats) {
					...
					buf +=
						...
						(isGenericFormat(this.curTeam.format)
							...
							: BattleLog.escapeFormat(
									BattleFormats[this.curTeam.format.slice(4)].name
							  )) +
						"</button>";
					var btnClass =
						"button" +
				...
				for (i = 0; i < this.curSetList.length; i++) {
					if (
						...
						buf +=
							'<li><button name="undeleteSet"><i class="fa fa-undo"></i> Undo Delete</button></li>';
					}
				...
				buf +=
					'<button name="pokepasteExport" type="submit" class="button exportbutton"><i class="fa fa-upload"></i> Upload to PokePaste</button></form>';
				if (this.curTeam.format.includes("vgc")) {
					buf +=
						'<button name="pokepasteExport" value="openteamsheet" type="submit" class="button exportbutton"><i class="fa fa-upload"></i> Upload to PokePaste (Open Team Sheet)</button></form>';
				}
				buf += "</div>";
			}
		...
		renderSet: function (set, i) {
			...
			var isNatDex =
				this.curTeam.format.includes("nationaldex") ||
				this.curTeam.format.includes("natdex");
			var buf = '<li value="' + i + '">';
			if (!set.species) {
				if (this.deletedSet) {
					buf +=
						'<div class="setmenu setmenu-left"><button name="undeleteSet"><i class="fa fa-undo"></i> Undo Delete</button></div>';
				}
				buf +=
				...
				buf +=
					...
					Dex.resourcePrefix +
					'sprites/gen5/0.png);"><div class="setcol setcol-icon"><div class="setcell-sprite"></div><div class="setcell setcell-pokemon"><label>Pok&eacute;mon</label><input type="text" name="pokemon" class="textbox chartinput" value="" autocomplete="off" /></div></div></div>';
				buf += "</li>";
				return buf;
			}
			buf +=
				'<div class="setmenu"><button name="copySet"><i class="fa fa-files-o"></i>Copy</button> <button name="importSet"><i class="fa fa-upload"></i>Import/Export</button> <button name="moveSet"><i class="fa fa-arrows"></i>Move</button> <button name="deleteSet"><i class="fa fa-trash"></i>Delete</button></div>';
			...
			buf += "</div>";
			buf +=
				'<div class="setchart" style="' +
				Dex.getTeambuilderSprite(set, this.curTeam.gen) +
				';">';
			...
			buf +=
				'<div class="setcell setcell-pokemon"><label>Pok&eacute;mon</label><input type="text" name="pokemon" class="textbox chartinput" value="' +
			...
			if (this.curTeam.gen > 1) {
				...
				if (isLetsGo) {
					buf +=
						...
						(typeof set.happiness === "number" ? set.happiness : 70) +
						"</span>";
		...
		changeFormat: function (format) {
			...
			if (
				(this.curTeam.gen === 5 ||
					this.curTeam.format.includes("eliteredux")) &&
		...
		clipboardHTML: function () {
			...
			if (this.curTeam && this.curSetList.length < this.curTeam.capacity) {
				buf +=
					'<button name="pastePokemon" class="teambuilder-clipboard-button-left"><i class="fa fa-clipboard"></i> Paste!</button>';
			}
			buf +=
				'<button name="clipboardRemoveAll" class="teambuilder-clipboard-button-right"><i class="fa fa-trash"></i> Clear clipboard</button>';
			buf += "</div>";
			buf += "</div>";
		...
		importSetButtons: function () {
			...
			var sets = $.extend(
				{},
				formatSets["dex"][species],
				formatSets["stats"][species]
			);

			$setDiv.text("Sample sets: ");
			for (var set in sets) {
				$setDiv.append(
					'<button name="importSmogonSet">' +
		...
		importSmogonSet: function (i, button) {
			...
			var smogonSet =
				formatSets["dex"][species][setName] ||
		...
		updateSetView: function () {
			...
			buf +=
				'<button name="back"><i class="fa fa-chevron-left"></i> Team</button></div>';
			...
			buf +=
				'<div class="pokemonedit-buttons"><button name="closePokemonImport"><i class="fa fa-chevron-left"></i> Back</button> <button name="savePokemonImport"><i class="fa fa-floppy-o"></i> Save</button></div>';
		...
		renderTeambar: function () {
			...
			for (var i = start; i < end; i++) {
				...
				if (!set.species) {
					buf +=
						'<button disabled="disabled" class="addpokemon" aria-label="Add Pok&eacute;mon"><i class="fa fa-plus"></i></button> ';
					isAdd = true;
				} else if (i == this.curSetLoc) {
					buf +=
						'<button disabled="disabled" class="pokemon">' +
						pokemonicon +
						BattleLog.escapeHTML(
							set.name ||
		...
		updateStatForm: function (setGuessed) {
			...
			buf +=
				'<div class="col evcol"><div><strong>' +
				(supportsEVs ? "EVs" : "AVs") +
				"</strong></div>";
			var totalev = 0;
			this.plus = "";
			...
			if (this.curTeam.gen > 2) {
				...
				if (hpType && !this.canHyperTrain(set)) {
					...
					buf +=
						'<div style="margin-left:-80px;text-align:right"><select name="ivspread">';
					buf +=
			...
			if (this.curTeam.gen > 2) {
				...
				buf +=
					'<p><em>Protip:</em> You can also set natures by typing "+" and "-" next to a stat.</p>';
			}
		...
		statChange: function (e) {
			...
			var supportsEVs = !this.curTeam.format.includes("letsgo");
			var supportsAVs =
				!supportsEVs && this.curTeam.format.endsWith("norestrictions");
			var set = this.curSet;
			if (!set) return;
		...
		updateDetailsForm: function () {
			...
			if (this.curTeam.gen > 1) {
				...
				if (species.gender && !isHackmons) {
					...
					if (!isHackmons) {
						buf +=
							'<label><input type="radio" name="gender" value="N"' +
							(!set.gender ? " checked" : "") +
							" /> Random</label>";
					} else {
						buf +=
				...
				if (isLetsGo) {
					buf +=
						'<div class="formrow"><label class="formlabel">Happiness:</label><div><input type="number" name="happiness" value="70" class="textbox inputform numform" /></div></div>';
				} else {
					if (this.curTeam.gen < 8 || isNatDex)
						buf +=
							'<div class="formrow"><label class="formlabel">Happiness:</label><div><input type="number" min="0" max="255" step="1" name="happiness" value="' +
							(typeof set.happiness === "number" ? set.happiness : 255) +
							'" class="textbox inputform numform" /></div></div>';
				}
				...
				buf +=
					'<label><input type="radio" name="shiny" value="yes"' +
					(set.shiny ? " checked" : "") +
					" /> Yes</label> ";
				buf +=
					'<label><input type="radio" name="shiny" value="no"' +
					(!set.shiny ? " checked" : "") +
					" /> No</label>";
				buf += "</div></div>";
				...
				if (this.curTeam.gen === 8 && !isBDSP) {
					...
					if (species.canGigantamax || species.forme === "Gmax") {
						...
						if (species.forme === "Gmax") {
							...
							buf +=
								'<label><input type="radio" name="gigantamax" value="yes"' +
								(set.gigantamax ? " checked" : "") +
								" /> Yes</label> ";
							buf +=
								'<label><input type="radio" name="gigantamax" value="no"' +
								(!set.gigantamax ? " checked" : "") +
								" /> No</label>";
						}
						buf += "</div></div>";
			...
			if (this.curTeam.gen > 2) {
				buf +=
					'<div class="formrow" style="display:none"><label class="formlabel">Pokeball:</label><div><select name="pokeball">';
				buf +=
					'<option value=""' +
			...
			if (
				...
				buf +=
					'<div class="formrow"><label class="formlabel" title="Hidden Power Type">Hidden Power:</label><div><select name="hptype">';
				buf +=
					'<option value=""' +
			...
			if (this.curTeam.gen === 9) {
				buf +=
					'<div class="formrow"><label class="formlabel" title="Tera Type">Tera Type:</label><div><select name="teratype">';
				var types = Dex.types.all();
				...
				for (var i = 0; i < types.length; i++) {
					buf +=
						...
						types[i].name +
						'"' +
						(teraType === types[i].name ? ' selected="selected"' : "") +
						">" +
						types[i].name +
						"</option>";
				}
		...
		detailsChange: function (e) {
			...
			if (this.curTeam.gen > 1) {
				...
				if (isLetsGo) {
					buf +=
						'<span class="detailcell"><label>Happiness</label>70</span>';
				} else {
					if (this.curTeam.gen < 8 || isNatDex)
						buf +=
							'<span class="detailcell"><label>Happiness</label>' +
				...
				buf +=
					...
					(set.shiny ? "Yes" : "No") +
					"</span>";
				if (!isLetsGo && (this.curTeam.gen < 8 || isNatDex))
					buf +=
						'<span class="detailcell"><label>HP Type</label>' +
						(set.hpType || "Dark") +
						"</span>";
		...
		chartClick: function (e) {
			if (this.search.addFilter(e.currentTarget)) {
				this.$("input[name=" + this.curChartName + "]")
					.val("")
					.select();
				this.search.find("");
				return;
		...
		chartChange: function (e, selectNext) {
			...
			switch (name) {
				case "pokemon":
					val =
						id in BattlePokedex
							? this.curTeam.dex.species.get(e.currentTarget.value).name
							: "";
					break;
				case "ability":
					if (
						id in BattleItems &&
						format &&
						format.endsWith("dualwielding")
					) {
						val = BattleItems[id].name;
					} else if (
						id in BattleMovedex &&
						format &&
						format.endsWith("trademarked")
					) {
				...
				case "item":
					if (
						id in BattleMovedex &&
						format &&
						format.endsWith("fortemons")
					) {
						val = BattleMovedex[id].name;
					} else if (
						id in BattleAbilities &&
						format &&
						format.endsWith("multibility")
					) {
		...
		chartSetCustom: function (val) {
			...
			if (val === "cathy") {
				...
				if (baseFormat.substr(0, 2) === "er")
					baseFormat = baseFormat.substr(2);
		...
		unChooseMove: function (moveName) {
			var set = this.curSet;
			if (!moveName || !set || this.curTeam.format === "gen7hiddentype")
				return;
		...
		canHyperTrain: function (set) {
			if (this.curTeam.gen < 7 || this.curTeam.format === "gen7hiddentype")
				return false;
			var format = this.curTeam.format;
		...
		setPokemon: function (val, selectNext) {
			...
			if (this.curTeam && this.curTeam.format) {
				...
				if (baseFormat.substr(0, 8) === "pokebank")
					baseFormat = baseFormat.substr(8);
				if (baseFormat.substr(0, 10) === "eliteredux")
					baseFormat = baseFormat.substr(10);
				if (this.curTeam && this.curTeam.format) {
					if (
						...
						set.level = 50;
					if (
						baseFormat.substr(0, 2) === "lc" ||
						baseFormat.substr(0, 5) === "caplc" ||
						baseFormat.substr(-2) === "lc"
					)
						set.level = 5;
			...
			for (var i = 0; i < data.team.length; i++) {
				...
				if (i !== data.i && i !== data.i + 1) {
					buf +=
						...
						i +
						'"><i class="fa fa-arrow-right"></i> Move here</button></li>';
				}
				buf +=
					"<li" +
		...
		initialize: function (data) {
			...
			for (var i = 0; i < formCount; i++) {
				...
				buf +=
					...
					'"';
				buf +=
					(form === species.form || (form === "" && !species.form)
						? ' class="cur"'
						: "") + "></button>";
			}
			buf += "</div>";
			...
			this.$el.html(buf).css({
				"max-width": (4 + spriteSize) * width,
				height: 42 + (4 + spriteSize) * height,
			});
		},
		setForm: function (form) {
			var species = Dex.species.get(this.curSet.species);
			if (form && form !== species.form) {
				this.curSet.species = Dex.species.get(
