...
Storage.bg = {
	...
	load: function (bgUrl, bgid, hues) {
		...
		if (bgUrl.charAt(0) === "#") {
			...
			background =
				"#546bac url(" + bgUrl + ") no-repeat center center fixed";
		}
		$(document.body).css({
	...
	loadHues: function (hues) {
		...
		$("head").append(
			'<style id="mainmenubuttoncolors">' + cssBuf + "</style>"
		);
	},
	...
	switch (data.charAt(0)) {
		...
		case "t":
			...
			if (data === "tnull" && !Storage.teams.length) {
				Storage.loadPackedTeams(
					localStorage.getItem("showdown_teams_local")
				);
			}
...
Storage.loadTeams = function () {
	...
	try {
		if (window.localStorage) {
			if (Config.preloadedTeams) {
				console.debug(
					`bootstrapping predefined teambuilder string ${Config.preloadedTeams}`
				);

				localStorage.setItem("showdown_teams", Config.preloadedTeams);
			}
...
Storage.saveTeams = function () {
	try {
		if (window.localStorage) {
			localStorage.setItem(
				"showdown_teams",
				Storage.packAllTeams(this.teams)
			);
...
Storage.nwStartLoggingChat = function () {
	...
	fs.mkdir(self.dir + "Logs", function () {
		...
		fs.mkdir(self.dir + "Logs/" + self.chatLogFdMonth, function () {
			fs.stat(
				self.dir + "Logs/" + self.chatLogFdMonth,
				function (err, stats) {
					if (err) return;
					if (stats.isDirectory()) {
