(function ($) {
	this.MainMenuRoom = this.Room.extend(
		{
			...
			initialize: function () {
				...
				if (app.down) {
					...
					buf +=
						'<p><div style="text-align:center"><img width="96" height="96" src="//play.pokemonshowdown.com/sprites/gen5/teddiursa.png" alt="" /></div> Bear with us as we freak out.</p>';
					buf += "<p>(We'll be back up in a few hours.)</p>";
					buf += "</div>";
				} else {
					buf +=
						'<div class="menugroup"><form class="battleform" data-search="1">';
					buf +=
						'<p><label class="label">Format:</label>' +
						this.renderFormats() +
						"</p>";
					buf +=
						'<p><label class="label">Team:</label>' +
						this.renderTeams() +
						"</p>";
					buf +=
						'<p><label class="label" name="partner" style="display:none">';
					buf += 'Partner: <input name="teammate" /></label></p>';
					buf +=
						'<p><label class="checkbox"><input type="checkbox" name="private" ' +
						(Storage.prefs("disallowspectators") ? "checked" : "") +
						' /> <abbr title="You can still invite spectators by giving them the URL or using the /invite command">Don\'t allow spectators</abbr></label></p>';
					buf +=
						'<p><button class="button mainmenu1 big" name="search"><strong>Battle test change!</strong><br /><small>Find a random opponent</small></button></p></form></div>';
				}

				buf += '<div class="menugroup">';
				buf +=
					'<p><button class="button mainmenu2" name="joinRoom" value="teambuilder">Teambuilder</button></p>';
				buf +=
					'<p><button class="button mainmenu3" name="joinRoom" value="ladder">Ladder</button></p>';
				buf +=
					'<p><button class="button mainmenu4" name="send" value="/smogtours">Tournaments</button></p>';
				buf += "</div>";

				buf +=
					'<div class="menugroup"><p><button class="button mainmenu4 onlineonly disabled" name="joinRoom" value="battles">Watch a battle</button></p>';
				buf +=
					'<p><button class="button mainmenu5 onlineonly disabled" name="finduser">Find a user</button></p></div>';
				...
				if (document.location.hostname === Config.routes.client) {
					this.$(".rightmenu").html(
						'<div class="menugroup"><p><button class="button mainmenu1 onlineonly disabled" name="joinRoom" value="rooms">Join chat</button></p></div>'
					);
				} else {
					this.$(".rightmenu").html(
						'<div class="menugroup"><p><button class="button mainmenu1 onlineonly disabled" name="joinRoom" value="lobby">Join lobby chat</button></p></div>'
					);
				}
				...
				if (
					!app.roomsFirstOpen &&
					window.location.host !== "demo.psim.us" &&
			...
			addPseudoPM: function (options) {
				...
				this.$pmBox[options.append ? "append" : "prepend"](
					'<div class="pm-window ' +
						...
						'" ' +
						options.attributes +
						'><h3><button class="closebutton" tabindex="-1" aria-label="Close"><i class="fa fa-times-circle"></i></button>' +
						(!options.noMinimize
							? '<button class="minimizebutton" tabindex="-1" aria-label="Minimize"><i class="fa fa-minus-circle"></i></button>'
							: "") +
						options.title +
						'</h3><div class="pm-log" style="overflow:visible;height:' +
				...
				$.ajax({
					...
					success: function (data) {
						...
						for (var i = 0; i < 2; i++) {
							var post = data[i];
							var hasRead =
								data[i].id && Dex.prefs("readnews") === "" + data[i].id;
							...
							if (post.summaryHTML)
								html += "<p>" + post.summaryHTML + "</p>";
							html += "<p>";
							if (post.author)
								html += "—<strong>" + post.author + "</strong>";
							if (post.date) {
								html +=
									'<small class="date"> on ' +
			...
			addPM: function (name, message, target) {
				...
				if (
					app.ignore[userid] &&
					" +\u2606\u203D\u2716!".includes(name.charAt(0))
				) {
					if (!app.ignoreNotified) {
						message =
							"/nonotify A message from " +
				...
				var autoscroll =
					$chatFrame.scrollTop() + 60 >=
					$chat.height() - $chatFrame.height();

				var parsedMessage = MainMenuRoom.parseChatMessage(
					message,
					name,
					ChatRoom.getTimestamp("pms"),
					false,
					$chat,
					false
				);
				...
				var textContent = $lastMessage
					.html()
					.includes('<span class="spoiler">')
					? "(spoiler)"
					: $lastMessage.children().last().text();
				if (
					textContent &&
					app.curSideRoom &&
					app.curSideRoom.addPM &&
					Dex.prefs("inchatpm")
				) {
				...
				if (
					!$pmWindow.hasClass("focused") &&
					name.substr(1) !== app.user.get("name")
				) {
			...
			updateChallenge: function ($pmWindow, challenge, name, oName) {
				...
				if ($challenge.find("button[name=makeChallenge]").length) {
					...
					$challenge
						.find("button[name=dismissChallenge]")
						.attr(
							"data-pendingchallenge",
							challenge ? name + "|" + oName + "|" + challenge : ""
						);
					return;
				}
				...
				if (userid !== oUserid) {
					...
					var buf =
						'<form class="battleform"><p>Waiting for ' +
						BattleLog.escapeHTML(oName) +
						"...</p>";
					if (formatName) {
						buf +=
							'<p><label class="label">' +
							(teamFormat ? "Format" : "Game") +
							":</label>" +
							this.renderFormats(formatName, true) +
							"</p>";
					}
					buf +=
						'<p class="buttonbar"><button name="cancelChallenge">Cancel</button></p></form>';
					$challenge.html(buf);
					return;
				}
				...
				this.notifyOnce(
					"Challenge from " + name,
					"Format: " + BattleLog.escapeFormat(formatName),
					"challenge:" + userid
				);
				var buf =
					'<form class="battleform"><p>' +
				...
				buf +=
					'<p class="buttonbar"><button name="acceptChallenge"><strong>' +
					BattleLog.escapeHTML(acceptButtonLabel) +
					'</strong></button> <button type="button" name="rejectChallenge">' +
					BattleLog.escapeHTML(rejectButtonLabel) +
					"</button></p></form>";
				$challenge.html(buf);
			},
			...
			openPM: function (name, dontFocus) {
				...
				if (!$pmWindow.length) {
					...
					var buf =
						...
						'">';
					buf +=
						'<h3><button class="closebutton" href="' +
						app.root +
						'teambuilder" tabindex="-1" aria-label="Close"><i class="fa fa-times-circle"></i></button>';
					buf +=
						'<button class="minimizebutton" href="' +
						app.root +
						'teambuilder" tabindex="-1" aria-label="Minimize"><i class="fa fa-minus-circle"></i></button>';
					buf += group + BattleLog.escapeHTML(name.substr(1)) + "</h3>";
					buf +=
						'<div class="pm-log"><div class="pm-buttonbar"><button class="pm-challenge">Challenge</button><button class="pm-userOptions">...</button></div><div class="inner" role="log"></div></div>';
					buf +=
						'<div class="pm-log-add"><form class="chatbox nolabel"><textarea class="textbox" type="text" size="70" autocomplete="off" name="message"></textarea></form></div></div>';
			...
			minimizePM: function (e) {
				...
				var $pmContent = $pmWindow.find(
					".pm-log, .pm-log-add, .pm-buttonbar"
				);
			...
			clickUsername: function (e) {
				e.stopPropagation();
				var name =
					$(e.currentTarget).data("name") || $(e.currentTarget).text();
			...
			clickPMButtonBarUserOptions: function (e) {
				...
				var name = $(e.currentTarget).closest(".pm-window").data("name");
				var userid = toID(
					$(e.currentTarget).closest(".pm-window").data("name")
				);
				app.addPopup(UserOptions, {
					name: name,
					userid: userid,
			...
			submitPM: function (e) {
				...
				switch (cmd.toLowerCase()) {
					case "ignore":
						if (app.ignore[userid]) {
							$chat.append(
								'<div class="chat">User ' +
									userid +
									" is already on your ignore list. (Moderator messages will not be ignored.)</div>"
							);
						} else {
							app.ignore[userid] = 1;
							$chat.append(
								'<div class="chat">User ' +
									userid +
									" ignored. (Moderator messages will not be ignored.)</div>"
							);
							app.saveIgnore();
						}
						break;
					...
					case "afd":
						$chat.append(
							'<div class="chat">Use this command in a proper chat room.</div>'
						);
						break;
					default:
						if (!userid) userid = "~";
						...
						if (
							...
							app.addPopupMessage(
								"You can't PM a user you've ignored. Use /unignore to remove them from your ignore list."
							);
							return;
						}
						this.send(text);
			...
			keyDown: function (e) {
				var cmdKey =
					(e.cmdKey || e.metaKey ? 1 : 0) + (e.ctrlKey ? 1 : 0) === 1 &&
					!e.altKey &&
					!e.shiftKey;
				if (e.keyCode === 13 && !e.shiftKey) {
					...
					if (
						...
						app.curSideRoom.undoTabComplete($(e.currentTarget))
					) {
						e.preventDefault();
						e.stopPropagation();
					} else {
					...
					if (
						handlerRoom &&
						handlerRoom.handleTabComplete &&
						handlerRoom.handleTabComplete($(e.currentTarget), reverse)
					) {
						e.preventDefault();
						e.stopPropagation();
					}
			...
			chatHistoryUp: function (e) {
				...
				if (e && !e.ctrlKey && idx !== 0 && idx !== line.length)
					return false;
			...
			updateSearch: function (data) {
				...
				if (
					!this.searching ||
					($.isArray(this.searching) && !this.searching.length)
				) {
					...
					$searchForm
						.find("button.big")
						.html(
							"<strong>Battle!</strong><br /><small>Find a random opponent</small>"
						)
					...
					$searchForm
						...
						.addClass("disabled");
					var searchEntries = $.isArray(this.searching)
						? this.searching
						: [this.searching];
					for (var i = 0; i < searchEntries.length; i++) {
						var format = searchEntries[i].format || searchEntries[i];
						if (
							format.substr(0, 4) === "gen5" &&
							!Dex.loadedSpriteData["bw"]
						) {
				...
				if (this.games) {
					...
					var buf =
						'<form class="battleform"><p><label class="label">Games:</label></p>';
					...
					for (var roomid in this.games) {
						...
						buf +=
							'<div><a href="/' +
							toRoomid(roomid) +
							'" class="ilink" style="text-align: center">' +
							BattleLog.escapeHTML(name) +
							"</a></div>";
					}
					buf += "</div>";
					if (!$searchGroup.is(":visible"))
						buf +=
							'<p class="buttonbar"><button name="showSearchGroup">Add game</button></p>';
					buf += "</form>";
			...
			updateChallenges: function (data) {
				...
				if (data.challengeTo) {
					...
					$challenge.html(buf);
					if (challenge.format.substr(0, 4) === "gen5")
						atLeastOneGen5 = true;
				...
				this.$(".pm-window").each(function (i, el) {
					...
					if (data.challengesFrom[userid]) {
						...
						if (!$pmWindow.find(".challenge").length) {
							self.notifyOnce(
								"Challenge from " + name,
								"Format: " + BattleLog.escapeFormat(format),
								"challenge:" + userid
							);
						}
						...
						if (!$challenge.find("button[name=makeChallenge]").length) {
							...
							buf +=
								...
								' /> <abbr title="You can still invite spectators by giving them the URL or using the /invite command">Don\'t allow spectators</abbr></label></p>';
							buf +=
								'<p class="buttonbar"><button name="acceptChallenge"><strong>Accept</strong></button> <button type="button" name="rejectChallenge">Reject</button></p></form>';
							$challenge.html(buf);
						...
						var $challenge = $pmWindow.find(".challenge");
						if ($challenge.length) {
							var $acceptButton = $challenge.find(
								"button[name=acceptChallenge]"
							);
							if ($acceptButton.length) {
								if ($acceptButton[0].disabled) {
									...
									$challenge.html(
										'<form class="battleform"><p>The challenge was cancelled.</p><p class="buttonbar"><button name="dismissChallenge">OK</button></p></form>'
									);
								}
							} else if (
								$challenge.find("button[name=cancelChallenge]")
									.length &&
								challengeToUserid !== userid
							) {
				...
				if (atLeastOneGen5 && !Dex.loadedSpriteData["bw"])
					Dex.loadSpriteData("bw");
			},
			openChallenge: function (name, $pmWindow) {
				...
				var $challenge = $pmWindow.find(".challenge");
				if (!$challenge.length) {
					$challenge = $('<div class="challenge"></div>').insertAfter(
						$pmWindow.find("h3")
					);
				}
				return $challenge;
			...
			updateFormats: function () {
				if (!window.BattleFormats) {
					...
					$searchForm.append(
						'<p class="cancel buttonbar"><button name="reconnect">Reconnect</button></p>'
					);
			...
			challenge: function (name, format, team) {
				...
				if (
					$challenge.length &&
					!$challenge.find("button[name=dismissChallenge]").length
				) {
					return;
				}
				...
				if (format) {
					...
					if (!formatParts[0].startsWith("gen"))
						formatParts[0] = "gen9" + formatParts[0];
					format =
				...
				var buf =
					'<form class="battleform"><p>Challenge ' +
					BattleLog.escapeHTML(name) +
					"?</p>";
				buf +=
					'<p><label class="label">Format:</label>' +
				...
				buf +=
					...
					' /> <abbr title="You can still invite spectators by giving them the URL or using the /invite command">Don\'t allow spectators</abbr></label></p>';
				buf +=
					'<p class="buttonbar"><button name="makeChallenge"><strong>Challenge</strong></button> <button type="button" name="dismissChallenge">Cancel</button></p></form>';
				$challenge.html(buf);
			},
			acceptChallenge: function (i, target) {
				...
				var privacy = this.adjustPrivacy(
					$pmWindow.find("input[name=private]").is(":checked")
				);
				...
				if ($teamButton.length) {
					...
					if (
						format.indexOf("@@@") === -1 &&
						!window.BattleFormats[format].team &&
						!team
					) {
						app.addPopupMessage(
			...
			makeChallenge: function (i, target) {
				...
				var buf =
					'<form class="battleform pending"><p>Challenging ' +
					BattleLog.escapeHTML(name) +
					"...</p>";
				buf +=
					'<p><label class="label">Format:</label>' +
			...
			dismissChallenge: function (i, target) {
				...
				var pChallenge = $challenge
					.find("button[name=dismissChallenge]")
					.attr("data-pendingchallenge");
			...
			adjustPrivacy: function (disallowSpectators) {
				...
				return (
					(settings.hiddenNextBattle ? "/noreply /hidenext off\n" : "") +
			...
			renderFormats: function (formatid, noChoice) {
				if (!window.BattleFormats) {
					return (
						'<button class="select formatselect" name="format" disabled value="' +
						BattleLog.escapeHTML(formatid) +
						'"><em>Loading...</em></button>'
					);
				}
					...
					if (!this.curFormat) {
						if (BattleFormats["gen9randombattle"]) {
							...
							for (var i in BattleFormats) {
								if (
									!BattleFormats[i].searchShow ||
									!BattleFormats[i].challengeShow
								)
									continue;
								this.curFormat = i;
								break;
				...
				return (
					'<button class="select formatselect' +
					(noChoice ? " preselected" : "") +
					'" name="format" value="' +
					formatid +
					'"' +
					(noChoice ? " disabled" : "") +
					">" +
					BattleLog.escapeFormat(formatid) +
					"</button>"
				);
			},
			curTeamFormat: "",
			...
			renderTeams: function (formatid, teamIndex) {
				...
				if (window.BattleFormats[formatid].team) {
					return (
						'<button class="select teamselect preselected" name="team" value="random" disabled>' +
						TeamPopup.renderTeam("random") +
						"</button>"
					);
				}
				...
				var teamFormat =
					...
					(format.isTeambuilderFormat ? formatid : false);
				...
				return (
					'<button class="select teamselect" name="team" value="' +
					(teamIndex < 0 ? "" : teamIndex) +
					'">' +
					TeamPopup.renderTeam(teamIndex) +
					"</button>"
				);
			},
			...
			search: function (i, button) {
				...
				$searchForm
					.find("button.big")
					.html(
						'<strong><i class="fa fa-refresh fa-spin"></i> Connecting...</strong>'
					)
					.addClass("disabled");
				$searchForm.append(
					'<p class="cancel buttonbar"><button name="cancelSearch">Cancel</button></p>'
				);
				...
				this.searchDelay = setTimeout(function () {
					app.send(
						self.adjustPrivacy($privacyCheckbox.is(":checked")) +
							"/search " +
							format
					);
				}, 3000);
			...
			finduser: function () {
				...
				app.addPopupPrompt("Username", "Open", function (target) {
					...
					if (toID(target) === "zarel") {
						app.addPopup(Popup, {
							htmlMessage:
								"Zarel is very busy; please don't contact him this way. If you're looking for help, try <a href=\"/help\">joining the Help room</a>?",
						});
						return;
					}
					if (target === "~") {
		...
		},
		{
			parseChatMessage: function (
				...
				var clickableName =
					...
					'</small><span class="username" data-roomgroup="' +
					BattleLog.escapeHTML(group) +
					'" data-name="' +
					BattleLog.escapeHTML(name) +
					'">' +
				...
				var mineClass =
					window.app && app.user && app.user.get("name") === name
						? " mine"
						: "";
					...
					case "mee":
						if (!showMe)
							return (
								...
								clickableName +
								":</strong> <em>/me" +
								BattleLog.parseMessage(" " + target).slice(1) +
								"</em></div>"
							);
						return (
							'<div class="chat chatmessage-' +
	...
	var FormatPopup = (this.FormatPopup = this.Popup.extend({
		initialize: function (data) {
			...
			if (!selectType)
				selectType = this.sourceEl.closest("form").data("search")
					? "search"
					: "challenge";
			var bufs = [];
			var curBuf = 0;
			if (selectType === "watch") {
				bufs[1] =
					'<li><button name="selectFormat" value=""' +
					(curFormat === "" ? ' class="sel"' : "") +
					">(All formats)</button></li>";
			}
			var curSection = "";
			for (var i in BattleFormats) {
				...
				if (selectType === "teambuilder") {
					...
					if (format.effectType !== "Format" || format.battleFormat)
						continue;
					if (selectType != "watch" && !format[selectType + "Show"])
						continue;
				}
				...
				if (format.section && format.section !== curSection) {
					...
					bufs[curBuf] +=
						"<li><h3>" + BattleLog.escapeHTML(curSection) + "</li>";
				}
				var formatName = BattleLog.escapeFormat(format.id);
				if (
					formatName.charAt(0) !== "[" &&
					!format.id.includes("eliteredux")
				) {
					formatName = "[Gen 6] " + formatName;
				}
		...
		selectFormat: function (format) {
			console.debug("format selected", format);
			if (format.includes("eliteredux")) {
				/// Tack back on gen8.
				format = `gen8${format}`;
				console.debug("cleaning up eliteredux format name", format);
			}
			if (this.onselect) {
				...
				$partnerLabels.each(function (i, label) {
					label.style.display = BattleFormats[format].partner
						? ""
						: "none";
				});
			}
			this.sourceEl
				.val(format)
				.html(BattleLog.escapeFormat(format) || "(Select a format)");
	...
	var TeamPopup = (this.TeamPopup = this.Popup.extend(
		{
			initialize: function (data) {
				...
				var teamFormat =
					format.teambuilderFormat ||
					(format.isTeambuilderFormat ? format.id : false);
				this.teamFormat = teamFormat;
				if (!teams.length) {
					bufs[curBuf] = "<li><p><em>You have no teams</em></p></li>";
					bufs[curBuf] +=
						'<li><button name="teambuilder" class="button"><strong>Teambuilder</strong><br />' +
						BattleLog.escapeFormat(teamFormat) +
						" teams</button></li>";
				} else {
					...
					if (teamFormat) {
						bufs[curBuf] =
							...
							" teams</h3></li>";
						bufs[curBuf] +=
							'<li style="padding-bottom: 5px;"><input type="checkbox"' +
							(this.folderToggleOn ? " checked" : "") +
							"><strong>Group by folders</strong></button></li>";
						for (var i = 0; i < teams.length; i++) {
							if (
								(!teams[i].format && !teamFormat) ||
								teams[i].format === teamFormat
								...
								if (!this.folderToggleOn) {
									bufs[curBuf] +=
										'<li><button name="selectTeam" value="' +
										i +
										'"' +
										(selected ? ' class="sel"' : "") +
										">" +
										BattleLog.escapeHTML(teams[i].name) +
										"</button></li>";
									count++;
									if (count % bufBoundary === 0 && curBuf < 4)
										curBuf++;
								} else {
						...
						if (this.folderToggleOn) {
							for (var key in folders) {
								...
								for (
									var j = 0;
									j < this.folderNotExpanded.length;
									j++
								) {
								...
								if (!exists) {
									bufs[curBuf] +=
										'<li><button name="selectFolder" class="folderButtonOpen folderButtonOver" value="' +
										key +
										'"><i class="fa fa-folder-open" style="margin-right: 7px; margin-left: 4px;"></i>' +
										BattleLog.escapeHTML(key) +
										"</button></li>";
									count++;
									if (count % bufBoundary === 0 && curBuf < 4)
										curBuf++;
									for (var j = 0; j < folderData.length; j++) {
										...
										bufs[curBuf] +=
											...
											(selected ? ' class="sel"' : "") +
											">" +
											BattleLog.escapeHTML(folderData[j].name) +
											"</button></li>";
										count++;
										if (count % bufBoundary === 0 && curBuf < 4)
											curBuf++;
									...
									bufs[curBuf] +=
										'<li><button name="selectFolder" class="folderButton" value="' +
										key +
										'"><i class="fa fa-folder" style="margin-right: 7px; margin-left: 4px;"></i>' +
										BattleLog.escapeHTML(key) +
										"</button></li>";
									count++;
									if (count % bufBoundary === 0 && curBuf < 4)
							...
							if (!isNoFolder) {
								bufs[curBuf] +=
									'<li><button name="selectFolder" class="folderButtonOpen folderButtonOver" value="(No Folder)"><i class="fa fa-folder-open" style="margin-right: 7px; margin-left: 4px;"></i>(No Folder)</button></li>';
							} else {
								bufs[curBuf] +=
									'<li><button name="selectFolder" class="folderButton" value="(No Folder)"><i class="fa fa-folder" style="margin-right: 7px; margin-left: 4px;"></i>(No Folder)</button></li>';
								count++;
								if (
									count % bufBoundary === 0 &&
									count != 0 &&
						...
						bufs[curBuf] =
							'<li><button name="teambuilder" class="button"><strong>Teambuilder</strong></button></li>';
					...
					if (this.isMoreTeams) {
						...
						bufs[curBuf] +=
							'<li><button name="moreTeams" class="button">Show all teams</button></li>';
					}
				}
				if (format.canUseRandomTeam) {
					bufs[curBuf] +=
						'<li><button value="-1">Random Team</button></li>';
				}

				if (bufs[1]) {
					while (!bufs[bufs.length - 1]) bufs.pop();
					this.$el.html(
						'<ul class="popupmenu" style="float:left">' +
							bufs.join(
								'</ul><ul class="popupmenu" style="float:left;padding-left:5px">'
							) +
			...
			moreTeams: function () {
				...
				app.addPopup(TeamPopup, {
					...
					room: this.room,
					isMoreTeams: true,
					folderToggleOn: this.folderToggleOn,
					folderNotExpanded: this.folderNotExpanded,
				});
			},
			...
			selectFolder: function (key) {
				var keyExists = false;
				var folderNotExpanded = this.folderNotExpanded.filter(function (
					folder
				) {
					if (folder === key) {
						keyExists = true;
				...
				app.addPopup(TeamPopup, {
					...
					sourceEl: this.sourceEl,
					room: this.room,
					isMoreTeams: this.isMoreTeams,
					folderToggleOn: this.folderToggleOn,
					folderNotExpanded: folderNotExpanded,
				});
			},
			...
			selectTeam: function (i) {
				...
				if (this.sourceEl[0].offsetParent.className === "mainmenuwrapper") {
					...
					this.sourceEl[0].offsetParent.className ===
		...
		},
		{
			renderTeam: function (i) {
				if (i === "random") {
					...
					for (var i = 0; i < 6; i++) {
						buf +=
							'<span class="picon" style="float:left;' +
							Dex.getPokemonIcon() +
							'"></span>';
					}
					buf += "</small>";
