...
export class BattleScene implements BattleSceneStub {
	...
	constructor(battle: Battle, $frame: JQuery, $logFrame: JQuery) {
		...
		if (battle.id) {
			numericId = parseInt(
				battle.id.slice(battle.id.lastIndexOf("-") + 1),
				10
			);
	...
	reset() {
		...
		this.$sprites = [
			$('<div class="sprite-back-0"></div>'),
			$('<div class="sprite-back-1"></div>'),
		];
		this.$spritesFront = [
			$('<div class="sprite-front-0"></div>'),
			$('<div class="sprite-front-1"></div>'),
		];
		...
		this.$stat = $(
			'<div role="complementary" aria-label="Active Pokemon"></div>'
		);
		this.$fx = $('<div class="fx-container"></div>');
		this.$leftbar = $(
			'<div class="leftbar" role="complementary" aria-label="Your Team"></div>'
		);
		this.$rightbar = $(
			'<div class="rightbar" role="complementary" aria-label="Opponent\'s Team"></div>'
		);
		this.$turn = $("<div></div>");
		this.$messagebar = $('<div class="messagebar message"></div>');
		this.$delay = $("<div></div>");
		this.$hiddenMessage = $(
			'<div class="message" style="position:absolute;display:block;visibility:hidden"></div>'
		);
		...
		if (!this.animating) {
			this.$battle.append(
				'<div class="seeking"><strong>seeking...</strong></div>'
			);
		}
	...
	pause() {
		...
		if (!this.battle.started) {
			this.$frame.append(
				'<div class="playbutton"><button name="play" class="button"><i class="fa fa-play"></i> Play</button><br /><br /><button name="play-muted" class="startsoundchooser button" style="font-size:10pt">Play (music off)</button></div>'
			);
			this.$frame
				.find("div.playbutton button[name=play-muted]")
				.click(() => {
					this.setMute(true);
					this.battle.play();
				});
		...
		this.$frame
			.find("div.playbutton button[name=play]")
			.click(() => this.battle.play());
	}
	resume() {
		this.$frame.find("div.playbutton").remove();
		this.updateBgm();
	...
	showEffect(
		...
		if (typeof effect === "string")
			effect = BattleEffects[effect] as SpriteData;
		...
		if (!end.xscale && end.xscale !== 0 && start.xscale)
			end.xscale = start.xscale;
		if (!end.yscale && end.yscale !== 0 && start.yscale)
			end.yscale = start.yscale;
		end = { ...start, ...end };
	...
	pos(loc: ScenePos, obj: SpriteData) {
		...
		let top = 245;
		let scale =
			obj.gen === 5 ? 2.0 - loc.z! / 200 : 1.5 - 0.5 * (loc.z! / 200);
		...
		let hoffset = Math.floor(
			(obj.h - (obj.y || 0) * 2) * scale * loc.yscale!
		);
	...
	posT(
		...
		if (transition === "ballistic") {
			transitionMap.top =
				pos.top < oldPos!.top ? "ballisticUp" : "ballisticDown";
		}
		if (transition === "ballisticUnder") {
			transitionMap.top =
				pos.top < oldPos!.top ? "ballisticDown" : "ballisticUp";
		}
	...
	closeMessagebar() {
		if (this.messagebarOpen) {
			...
			if (this.animating) {
				this.$messagebar
					.delay(this.battle.messageShownTime / this.acceleration)
					.animate(
						{
							opacity: 0,
						},
	...
	runMoveAnim(moveid: ID, participants: Pokemon[]) {
		...
		if (this.acceleration >= 3) {
			...
			const isSpecial =
				!targetsSelf &&
				this.battle.dex.moves.get(moveid).category === "Special";
			animEntry =
				BattleOtherAnims[
					targetsSelf
						? "fastanimself"
		...
		animEntry.anim(
			this,
			participants.map((p) => p.sprite)
		);
	}

	runOtherAnim(moveid: ID, participants: Pokemon[]) {
		if (!this.animating) return;
		BattleOtherAnims[moveid].anim(
			this,
			participants.map((p) => p.sprite)
		);
	}
	...
	updateGen() {
		...
		const isSPL =
			typeof this.battle.rated === "string" &&
			this.battle.rated.startsWith("Smogon Premier League");
		let bg: string;
		if (isSPL) {
			...
			if (gen <= 1) bg = "fx/bg-gen1.png?";
				...
				bg =
					"fx/" +
					BattleBackdropsThree[
						this.numericId % BattleBackdropsThree.length
					] +
					"?";
			else if (gen <= 4)
				bg =
					"fx/" +
					BattleBackdropsFour[this.numericId % BattleBackdropsFour.length];
			else if (gen <= 5)
				bg =
					"fx/" +
					BattleBackdropsFive[this.numericId % BattleBackdropsFive.length];
			else
				bg =
					"sprites/gen6bgs/" +
					BattleBackdrops[this.numericId % BattleBackdrops.length];
		}
		...
		if (this.$bg) {
			this.$bg.css(
				"background-image",
				"url(" + Dex.resourcePrefix + "" + this.backdropImage + ")"
			);
		}
	}
	...
	getSidebarHTML(side: Side, posStr: string): string {
		...
		let speciesOverage = this.battle.speciesClause
			? Infinity
			: Math.max(side.pokemon.length - side.totalPokemon, 0);
		const sidebarIcons: (
			...
			for (let i = 0; i < side.pokemon.length; i++) {
				...
				if (speciesOverage && speciesTable.includes(species)) {
					for (const sidebarIcon of sidebarIcons) {
						if (
							side.pokemon[sidebarIcon[1]!].getBaseSpecies()
								.baseSpecies === species
						) {
		...
		if (
			!zoroarkRevealed &&
			hasIllusion &&
			sidebarIcons.length < side.totalPokemon
		) {
		...
		for (let i = 0; i < sidebarIcons.length; i++) {
			...
			const tooltipCode = ` class="picon has-tooltip" data-tooltip="pokemon|${
				side.n
			}|${pokeIndex}${iconType === "pokemon-illusion" ? "|illusion" : ""}"`;
			if (iconType === "empty") {
				pokemonhtml +=
					`<span class="picon" style="` +
					Dex.getPokemonIcon("pokeball-none") +
					`"></span>`;
			} else if (noShow) {
				if (poke?.fainted) {
					pokemonhtml +=
						`<span${tooltipCode} style="` +
						Dex.getPokemonIcon("pokeball-fainted") +
						`" aria-label="Fainted"></span>`;
				} else if (poke?.status) {
					pokemonhtml +=
						`<span${tooltipCode} style="` +
						Dex.getPokemonIcon("pokeball-statused") +
						`" aria-label="Statused"></span>`;
				} else {
					pokemonhtml +=
						`<span${tooltipCode} style="` +
						Dex.getPokemonIcon("pokeball") +
						`" aria-label="Non-statused"></span>`;
				}
			} else if (iconType === "pseudo-zoroark") {
				pokemonhtml +=
					`<span class="picon" style="` +
					Dex.getPokemonIcon("zoroark") +
					`" title="Unrevealed Illusion user" aria-label="Unrevealed Illusion user"></span>`;
			} else if (!poke) {
				pokemonhtml +=
					`<span class="picon" style="` +
					Dex.getPokemonIcon("pokeball") +
					`" title="Not revealed" aria-label="Not revealed"></span>`;
			} else if (
				!poke.ident &&
				this.battle.teamPreviewCount &&
				this.battle.teamPreviewCount < side.pokemon.length
			) {
				...
				pokemonhtml +=
					`<span${tooltipCode} style="` +
					Dex.getPokemonIcon(poke, !side.isFar) +
					`;opacity:0.6" aria-label="${details}"></span>`;
			} else {
				const details = this.getDetailsText(poke);
				pokemonhtml +=
					`<span${tooltipCode} style="` +
		...
		const ratinghtml = side.rating
			? ` title="Rating: ${BattleLog.escapeHTML(side.rating)}"`
			: ``;
		const faded = side.name ? `` : ` style="opacity: 0.4"`;
		return `<div class="trainer trainer-${posStr}"${faded}><strong>${BattleLog.escapeHTML(
			side.name
		)}</strong><div class="trainersprite"${ratinghtml} style="background-image:url(${Dex.resolveAvatar(
			side.avatar
		)})"></div>${pokemonhtml}</div>`;
	}
	updateSidebar(side: Side) {
		if (this.battle.gameType === "freeforall") {
			...
			side === this.battle.nearSide ||
			side === this.battle.nearSide.ally
		) {
			this.updateLeftSidebar();
		} else {
			this.updateRightSidebar();
	...
	updateLeftSidebar() {
		...
		if (side.ally) {
			...
			this.$leftbar.html(
				this.getSidebarHTML(side, "near2") +
					this.getSidebarHTML(side2, "near")
			);
		} else if (this.battle.sides.length > 2) {
			// FFA
			const side2 = this.battle.sides[side.n === 0 ? 3 : 2];
			this.$leftbar.html(
				this.getSidebarHTML(side2, "near2") +
					this.getSidebarHTML(side, "near")
			);
		} else {
	...
	updateRightSidebar() {
		...
		if (side.ally) {
			const side2 = side.ally!;
			this.$rightbar.html(
				this.getSidebarHTML(side, "far2") +
					this.getSidebarHTML(side2, "far")
			);
		} else if (this.battle.sides.length > 2) {
			// FFA
			const side2 = this.battle.sides[side.n === 0 ? 3 : 2];
			this.$rightbar.html(
				this.getSidebarHTML(side2, "far2") +
					this.getSidebarHTML(side, "far")
			);
		} else {
		...
		for (const side of this.battle.sides) {
			...
			side.missedPokemon = {
				sprite: new PokemonSprite(
					...
					{
						x: side.leftof(
							this.battle.gameType === "freeforall" ? -50 : -100
						),
						y: side.y,
						z: side.z,
						opacity: 0,
	...
	teamPreview() {
		...
		for (
			let siden = 0;
			siden < 2 || (this.battle.gameType === "multi" && siden < 4);
			siden++
		) {
			...
			for (let i = 0; i < side.pokemon.length; i++) {
				...
				buf +=
					...
					Math.floor(y - spriteData.h / 2) +
					"px;left:" +
					Math.floor(x - spriteData.w / 2) +
					'px" />';
				buf2 +=
					...
					"px;left:" +
					(x - 40) +
					'px;width:80px;font-size:10px;text-align:center;color:#FFF;">';
				const gender = pokemon.gender;
				if (gender === "M" || gender === "F") {
					buf2 += `<img src="${
						Dex.fxPrefix
					}gender-${gender.toLowerCase()}.png" alt="${gender}" width="7" height="10" class="pixelated" style="margin-bottom:-1px" /> `;
				}
				if (pokemon.level !== 100) {
					buf2 +=
						'<span style="text-shadow:#000 1px 1px 0,#000 1px -1px 0,#000 -1px 1px 0,#000 -1px -1px 0"><small>L</small>' +
						pokemon.level +
						"</span>";
				}
				if (pokemon.item === "(mail)") {
					buf2 +=
						' <img src="' +
						Dex.resourcePrefix +
						'fx/mail.png" width="8" height="10" alt="F" style="margin-bottom:-1px" />';
				} else if (pokemon.item) {
					buf2 +=
						' <img src="' +
						Dex.resourcePrefix +
						'fx/item.png" width="8" height="10" alt="F" style="margin-bottom:-1px" />';
				}
				buf2 += "</div>";
			}
			...
			if (textBuf) {
				this.log.addDiv(
					...
					"<strong>" +
						...
						'\'s team:</strong> <em style="color:#445566;display:block;">' +
						BattleLog.escapeHTML(textBuf) +
						"</em>"
				);
			}
	...
	showJoinButtons() {
		...
		if (!this.battle.p1.name) {
			this.$battle.append(
				'<div class="playbutton1"><button name="joinBattle">Join Battle</button></div>'
			);
		}
		if (!this.battle.p2.name) {
			this.$battle.append(
				'<div class="playbutton2"><button name="joinBattle">Join Battle</button></div>'
			);
		}
	}
	hideJoinButtons() {
	...
	sideConditionLeft(
		cond: [string, number, number, number],
		isFoe: boolean,
		all?: boolean
	) {
		if (!cond[2] && !cond[3] && !all) return "";
		let buf = `<br />${isFoe && !all ? "Foe's " : ""}${
			Dex.moves.get(cond[0]).name
		}`;
		if (this.battle.gen < 7 && this.battle.hardcoreMode) return buf;
	...
	weatherLeft() {
		...
		if (this.battle.weather) {
			...
			weatherhtml = `${
				weatherNameTable[this.battle.weather] || this.battle.weather
			}`;
			if (this.battle.weatherMinTimeLeft !== 0) {
				...
				weatherhtml += ` <small>(${this.battle.weatherTimeLeft} turn${
					this.battle.weatherTimeLeft === 1 ? "" : "s"
				})</small>`;
			}
			const nullifyWeather = this.battle.abilityActive([
				"Air Lock",
				"Cloud Nine",
				"Clueless",
			]);
			weatherhtml = `${nullifyWeather ? "<s>" : ""}${weatherhtml}${
				nullifyWeather ? "</s>" : ""
			}`;
		}
	...
	sideConditionsLeft(side: Side, all?: boolean) {
		...
		for (const id in side.sideConditions) {
			buf += this.sideConditionLeft(
				side.sideConditions[id],
				side.isFar,
				all
			);
		}
	...
	updateWeather(instant?: boolean) {
		...
		if (instant) {
			...
			this.$terrain.attr(
				"class",
				terrain ? "weather " + terrain + "weather" : "weather"
			);
	...
	resetTurn() {
		...
		this.$turn.html(
			'<div class="turn has-tooltip" data-tooltip="field" data-ownheight="1">Turn ' +
				this.battle.turn +
				"</div>"
		);
	}
	...
	updateAcceleration() {
		if (this.battle.turnsSinceMoved > 2) {
			this.acceleration =
				(this.battle.messageFadeTime < 150 ? 2 : 1) *
				Math.min(this.battle.turnsSinceMoved - 1, 3);
		} else {
	...
	addPokemonSprite(pokemon: Pokemon) {
		const sprite = new PokemonSprite(
			Dex.getSpriteData(pokemon, pokemon.side.isFar, {
				gen: this.gen,
				mod: this.mod,
			}),
			{
	...
	addSideCondition(siden: number, id: ID, instant?: boolean) {
		...
		switch (id) {
			case "auroraveil":
				const auroraveil = new Sprite(
					BattleEffects.auroraveil,
					{
						display: "block",
						x,
						y,
			...
			case "reflect":
				...
				break;
			case "safeguard":
				const safeguard = new Sprite(
					BattleEffects.safeguard,
					{
						display: "block",
						x,
						y,
				...
				break;
			case "lightscreen":
				const lightscreen = new Sprite(
					BattleEffects.lightscreen,
					{
						display: "block",
						x,
						y,
			...
			case "stealthrock":
				...
				break;
			case "gmaxsteelsurge":
				const surge1 = new Sprite(
					BattleEffects.greenmetal1,
					{
						display: "block",
						x: x + side.leftof(-30),
						y: y - 20,
				...
				const surge2 = new Sprite(
					BattleEffects.greenmetal2,
					{
						display: "block",
						x: x + side.leftof(35),
						y: y - 15,
			...
			case "toxicspikes":
				...
				let tspikeLevels =
					this.battle.sides[siden].sideConditions["toxicspikes"][1];
				if (tspikeArray.length < 1 && tspikeLevels >= 1) {
					const tspike1 = new Sprite(
	...
	typeAnim(pokemon: Pokemon, types: string) {
		const result = BattleLog.escapeHTML(types)
			.split("/")
			.map(
				(type) =>
					'<img src="' +
	...
	resultAnim(
		pokemon: Pokemon,
		result: string,
		type: "bad" | "good" | "neutral" | StatusName
	) {
		if (!this.animating) return;
		let $effect = $(
			'<div class="result ' +
	...
	abilityActivateAnim(pokemon: Pokemon, result: string) {
		if (!this.animating) return;
		this.$fx.append(
			'<div class="result abilityresult"><strong>' +
				result +
				"</strong></div>"
		);
	...
	damageAnim(pokemon: Pokemon, damage: number | string) {
		...
		this.resultAnim(
			pokemon,
			this.battle.hardcoreMode ? "Damage" : "&minus;" + damage,
			"bad"
		);
	...
	healAnim(pokemon: Pokemon, damage: number | string) {
		...
		this.resultAnim(
			pokemon,
			this.battle.hardcoreMode ? "Heal" : "+" + damage,
			"good"
		);
	...
	updateStatbarIfExists(
		pokemon: Pokemon,
		updatePrevhp?: boolean,
		updateHp?: boolean
	) {
		return pokemon.sprite.updateStatbarIfExists(
			pokemon,
			updatePrevhp,
			updateHp
		);
...
export class Sprite {
	...
	anim(end: ScenePos, transition?: string) {
		...
		this.$el!.animate(
			this.scene.posT(end, this.sp, transition, this),
			end.time!
		);
		return this;
	}
...
export class PokemonSprite extends Sprite {
	...
	protected static statusTable: {
		[id: string]: [string, "good" | "bad" | "neutral"] | null | string;
	} = {
		formechange: null,
		typechange: null,
		typeadd: null,
	...
	animSub(instant?: boolean) {
		...
		this.$sub = $(
			'<img src="' +
				subsp.url +
				'" style="display:block;opacity:0;position:absolute"' +
				(subsp.pixelated ? ' class="pixelated"' : "") +
				" />"
		);
	...
	reset(pokemon: Pokemon) {
		...
		if (
			pokemon.volatiles.formechange ||
			pokemon.volatiles.dynamax ||
			pokemon.volatiles.terastallize
		) {
		...
		if (this.$el) {
			...
			const $newEl = $(
				'<img src="' +
					this.sp.url +
					'" style="display:none;position:absolute"' +
					(this.sp.pixelated ? ' class="pixelated"' : "") +
					" />"
			);
			this.$el = $newEl;
		}
	...
	recalculatePos(slot: number) {
		...
		if (this.scene.gen <= 4 && moreActive) {
			...
			if (this.isFrontSprite && !moreActive && this.sp.pixelated)
				statbarOffset = 15;
			if (!this.isFrontSprite) statbarOffset = -7 * slot;
			if (this.isFrontSprite && moreActive === 2)
				statbarOffset = 14 * slot - 10;
		}
		if (this.scene.gen <= 2) {
	...
	animSummon(pokemon: Pokemon, slot: number, instant?: boolean) {
		...
		if (this.sp.shiny && this.scene.acceleration < 2)
			BattleOtherAnims.shiny.anim(this.scene, [this]);
		this.scene.waitFor(this.$el);
	...
	animDragIn(pokemon: Pokemon, slot: number) {
		...
		if (!!this.scene.animating && this.sp.shiny)
			BattleOtherAnims.shiny.anim(this.scene, [this]);
		this.scene.waitFor(this.$el);
	...
	animUnsummon(pokemon: Pokemon, instant?: boolean) {
		...
		if (this.scene.acceleration < 3)
			this.scene.wait(600 / this.scene.acceleration);
	...
	addEffect(id: ID, instant?: boolean) {
		...
		if (id === "substitute" || id === "shedtail") {
			...
			const leechseed1 = new Sprite(
				BattleEffects.energyball,
				pos1,
				this.scene
			);
			const leechseed2 = new Sprite(
				BattleEffects.energyball,
				pos2,
				this.scene
			);
			const leechseed3 = new Sprite(
				BattleEffects.energyball,
				pos3,
				this.scene
			);
			...
			const protect = new Sprite(
				BattleEffects.protect,
				{
					display: "block",
					x: this.x,
					y: this.y,
	...
	getStatbarHTML(pokemon: Pokemon) {
		let buf =
			'<div class="statbar' +
			(this.isFrontSprite ? " lstatbar" : " rstatbar") +
			this.getClassForPosition(pokemon.slot) +
			'" style="display: none">';
		const ignoreNick =
			this.isFrontSprite &&
			(this.scene.battle.ignoreOpponent || this.scene.battle.ignoreNicks);
		buf += `<strong>${BattleLog.escapeHTML(
			ignoreNick ? pokemon.speciesForme : pokemon.name
		)}`;
		const gender = pokemon.gender;
		if (gender === "M" || gender === "F") {
			buf += ` <img src="${
	...
	updateStatbar(pokemon: Pokemon, updatePrevhp?: boolean, updateHp?: boolean) {
		...
		if (updatePrevhp || updateHp) {
			...
			if (hpcolor === "g") $hp.removeClass("hp-yellow hp-red");
				...
				$hp.removeClass("hp-red").addClass("hp-yellow");
		...
		if (updatePrevhp) {
			...
			if (hpcolor === "g") $prevhp.removeClass("prevhp-yellow prevhp-red");
				...
				$prevhp.removeClass("prevhp-red").addClass("prevhp-yellow");
		...
		if (pokemon.terastallized) {
			status += `<img src="${
				Dex.resourcePrefix
			}sprites/types/${encodeURIComponent(
				pokemon.terastallized
			)}.png" alt="${pokemon.terastallized}" class="pixelated" /> `;
		} else if (
			pokemon.volatiles.typechange &&
			pokemon.volatiles.typechange[1]
		) {
		...
		for (const stat in pokemon.boosts) {
			if (pokemon.boosts[stat]) {
				status +=
					'<span class="' +
					pokemon.getBoostType(stat as BoostStatName) +
					'">' +
					pokemon.getBoost(stat as BoostStatName) +
					"</span> ";
			}
		}
		...
		for (let i in pokemon.turnstatuses) {
			if (i === "roost" && !pokemon.getTypeList().includes("Flying"))
				continue;
			status += PokemonSprite.getEffectTag(i);
		}
	...
	private static getEffectTag(id: string) {
		...
		return (PokemonSprite.statusTable[id] = `<span class="${
			effect[1]
		}">${effect[0].replace(/ /g, "&nbsp;")}</span> `);
	}
	...
	updateHPText(pokemon: Pokemon) {
		...
		if (
			pokemon.maxhp === 48 ||
			(this.scene.battle.hardcoreMode && pokemon.maxhp === 100)
		) {
			$hptext.hide();
			$hptextborder.hide();
...
const BattleEffects: { [k: string]: SpriteData } = {
	...
	protect: {
		rawHTML:
			'<div class="turnstatus-protect" style="display:none;position:absolute" />',
		w: 100,
		h: 70,
	},
	auroraveil: {
		rawHTML:
			'<div class="sidecondition-auroraveil" style="display:none;position:absolute" />',
		w: 100,
		h: 50,
	},
	reflect: {
		rawHTML:
			'<div class="sidecondition-reflect" style="display:none;position:absolute" />',
		w: 100,
		h: 50,
	},
	safeguard: {
		rawHTML:
			'<div class="sidecondition-safeguard" style="display:none;position:absolute" />',
		w: 100,
		h: 50,
	},
	lightscreen: {
		rawHTML:
			'<div class="sidecondition-lightscreen" style="display:none;position:absolute" />',
		w: 100,
		h: 50,
	},
	mist: {
		rawHTML:
			'<div class="sidecondition-mist" style="display:none;position:absolute" />',
		w: 100,
		h: 50,
	},
};
...
export const BattleOtherAnims: AnimTable = {
	...
	spectralthiefboost: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect(
				"linear-gradient(#000000 30%, #440044",
				1400,
				0.5
			);
			scene.showEffect(
	...
	primalomega: {
		anim(scene, [attacker]) {
			scene.backgroundEffect(
				"linear-gradient(#390000 30%, #B84038)",
				500,
				0.4
			);
			scene.showEffect(
	...
	zpower: {
		anim(scene, [attacker]) {
			scene.backgroundEffect(
				"linear-gradient(#000000 20%, #0000DD)",
				1800,
				0.4
			);
			scene.showEffect(
