...
class DexSearch {
	...
	textSearch(query: string): SearchRow[] {
		...
		if (query in BattleAliases) {
			if (
				...
				let aliasPassType: SearchPassType =
					queryAlias === "hiddenpower" ? "exact" : "normal";
		...
		for (i = 0; i < BattleSearchIndex.length; i++) {
			...
			if (passType === "alias") {
				...
				if (matchEnd)
					matchEnd +=
						(BattleSearchIndexOffset[i][matchEnd - 1] || "0").charCodeAt(
							0
						) - 48;
			}
		...
		if (instafilter && count < 20) {
			...
			bufs.push(
				this.instafilter(searchType, instafilter[0], instafilter[1])
			);
		}
	...
	private instafilter(
		...
		if (searchType === "pokemon") {
			switch (fType) {
				case "type":
					let type = (fId.charAt(0).toUpperCase() +
						fId.slice(1)) as TypeName;
...
class BattlePokemonSearch extends BattleTypedSearch<"pokemon"> {
	...
	getBaseResults(): SearchRow[] {
		...
		if ((format.endsWith("cap") || format.endsWith("caplc")) && dex.gen < 9) {
			...
			table =
				table["gen" + dex.gen + "stadium" + (dex.gen > 1 ? dex.gen : "")];
		}
		if (!table.tierSet) {
		...
		if (format.endsWith("ubers") || format.endsWith("uber"))
			...
			this.formatType?.startsWith("bdsp") ||
			this.formatType === "letsgo" ||
			this.formatType === "stadium"
		) {
			tierSet = tierSet.slice(slices.Uber);
	...
	filter(row: SearchRow, filters: string[][]) {
		...
		for (const [filterType, value] of filters) {
			switch (filterType) {
				...
				case "egggroup":
					if (
						species.eggGroups[0] !== value &&
						species.eggGroups[1] !== value
					)
						return false;
					break;
				case "tier":
	...
	sort(results: SearchRow[], sortCol: string, reverseSort?: boolean) {
		...
		if (["hp", "atk", "def", "spa", "spd", "spe"].includes(sortCol)) {
			return results.sort(([rowType1, id1], [rowType2, id2]) => {
				const stat1 =
					this.dex.species.get(id1).baseStats[sortCol as StatName];
				const stat2 =
					this.dex.species.get(id2).baseStats[sortCol as StatName];
...
class BattleAbilitySearch extends BattleTypedSearch<"ability"> {
	...
	getBaseResults() {
		...
		if (species.isMega) {
			abilitySet.unshift([
				"html",
				`Will be <strong>${species.abilities["0"]}</strong> after Mega Evolving.`,
			]);
		...
		if (isER) {
			...
			if (species.abilities["I2"]) {
				abilitySet.push(["ability", toID(species.abilities["I2"])]);
			}
			if (species.abilities["I3"]) {
				abilitySet.push(["ability", toID(species.abilities["I3"])]);
			}
		}
		if (isAAA || format.includes("metronomebattle") || isHackmons) {
			...
			for (const ability of abilities
				.sort()
				.map((abil) => dex.abilities.get(abil))) {
...
class BattleMoveSearch extends BattleTypedSearch<"move"> {
	...
	private moveIsNotUseless(
		...
		if (dex.gen === 1) {
			...
			switch (id) {
				...
				case "reflect":
					return (
						!moves.includes("barrier") && !moves.includes("acidarmor")
					);
				case "stomp":
		...
		switch (id) {
			...
			case "jumpkick":
				return (
					!moves.includes("highjumpkick") && !moves.includes("axekick")
				);
			case "lastresort":
			...
			case "phantomforce":
				return (
					(!moves.includes("poltergeist") &&
						!moves.includes("shadowclaw")) ||
					this.formatType === "doubles"
				);
			case "poisonfang":
				return (
			...
			case "refresh":
				return (
					!moves.includes("aromatherapy") && !moves.includes("healbell")
				);
			case "risingvoltage":
			...
			case "shelter":
				return (
					!moves.includes("acidarmor") && !moves.includes("irondefense")
				);
			case "smackdown":
				return species.types.includes("Ground");
			case "smartstrike":
				return (
					species.types.includes("Steel") && !moves.includes("ironhead")
				);
			case "soak":
			...
			case "technoblast":
				return (
					(dex.gen > 5 && itemid.endsWith("drive")) ||
					itemid === "dousedrive"
				);
			case "teleport":
				return dex.gen > 7;
	...
	getBaseResults() {
		...
		if (this.formatType?.startsWith("eliteredux"))
			lsetTable = lsetTable["eliteredux"];
		...
		while (learnsetid) {
			...
			if (learnset) {
				for (let moveid in learnset) {
					...
					const minGenCode: { [gen: number]: string } = {
						6: "p",
						7: "q",
						8: "g",
						9: "a",
					...
					if (
						(this.formatType !== "natdex" || "eliteredux") &&
						move.isNonstandard === "Past"
					) {
						continue;
					}
					if (
						...
						BattleTeambuilderTable["gen8dlc1"]?.nonstandardMoves.includes(
							moveid
						)
					) {
						continue;
		...
		if (isSTABmons) {
			for (let id in this.getTable()) {
				...
				for (let i = dex.gen; i >= species.gen && i >= move.gen; i--) {
					...
					let baseSpecies = genDex.species.get(
					...
					if (
						...
						if (!excludedForme(species))
							speciesTypes.push(...baseSpecies.types);
		...
		for (const id of moves) {
			const isUsable = this.moveIsNotUseless(
				id as ID,
				species,
				moves,
				this.set
