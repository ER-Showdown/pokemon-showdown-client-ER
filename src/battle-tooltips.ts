...
class ModifiableValue {
	...
	constructor(battle: Battle, pokemon: Pokemon, serverPokemon: ServerPokemon) {
		...
		const ability =
			serverPokemon.ability || pokemon?.ability || serverPokemon.baseAbility;
		this.abilityName = Dex.abilities.get(ability).name;
		this.innates = this.setInnates(pokemon, serverPokemon);
		this.weatherName =
			battle.weather === "snow"
				? "Snow"
				: Dex.moves.get(battle.weather).exists
				? Dex.moves.get(battle.weather).name
				: Dex.abilities.get(battle.weather).name;
	}
	setInnates(
		pokemon: Pokemon | null,
		serverPokemon: ServerPokemon | null | undefined
	) {
		//TODO: Make more efficient. Function stresses client out
		let innateList = [];
		if (pokemon) {
			const speciesForme =
				pokemon.getSpeciesForme() || serverPokemon?.speciesForme || "";
			const species = this.battle.dex.species.get(speciesForme);
			if (species.exists && species.abilities["I1"]) {
				if (species.abilities["I1"])
					innateList.push(species.abilities["I1"]);
				if (species.abilities["I2"])
					innateList.push(species.abilities["I2"]);
				if (species.abilities["I3"])
					innateList.push(species.abilities["I3"]);
			}
		}
		return innateList;
	}
	...
	tryAbility(abilityName: string) {
		...
		if (
			...
			!this.innates.includes(abilityName)
		)
			return false;
	...
	tryWeather(weatherName?: string) {
		...
		for (const side of this.battle.sides) {
			for (const active of side.active) {
				if (
					active &&
					["Air Lock", "Cloud Nine", "Clueless"].includes(active.ability)
				) {
					this.comment.push(
						` (${weatherName} suppressed by ${active.ability})`
					);
					return false;
				}
			}
	...
	modify(factor: number, name?: string) {
		...
		if (name)
			this.comment.push(` (${this.round(factor)}&times; from ${name})`);
		this.value *= factor;
		if (!(name === "Technician" && this.maxValue > 60))
	...
	toString() {
		...
		if (this.maxValue) {
			valueString +=
				` to ${this.round(this.maxValue)}` + (this.isAccuracy ? "%" : "");
		}
...
class BattleTooltips {
	...
	listen(elem: HTMLElement | JQuery<HTMLElement>) {
		...
		$elem.on("touchend", ".has-tooltip", (e) => {
			e.preventDefault();
			if (
				e.currentTarget === BattleTooltips.parentElem &&
	...
	holdLockTooltipEvent = (e: JQuery.TriggeredEvent) => {
		...
		let factor =
			e.type === "mousedown" && target.tagName === "BUTTON" ? 2 : 1;
	...
	showTooltip(elem: HTMLElement) {
		...
		switch (type) {
			...
			case "activepokemon": {
				...
				let sideIndex = parseInt(args[1], 10);
				let side =
					this.battle.sides[+this.battle.sidesSwitched ^ sideIndex];
			...
			case "allypokemon": {
				...
				let serverPokemon = this.battle.myAllyPokemon
					? this.battle.myAllyPokemon[activeIndex]
					: null;
				buf = this.showPokemonTooltip(pokemon, serverPokemon);
				break;
			}
			...
			default:
				...
				buf = `<p class="message-error" style="white-space: pre-wrap">${
	...
	showMoveTooltip(
		...
		if (this.battle.gameType === "freeforall") {
			foeActive = [...foeActive, ...pokemon.side.active].filter(
				(active) => active !== pokemon
			);
		}
		// TODO: move this somewhere it makes more sense
		if (pokemon.ability === "(suppressed)")
			serverPokemon.ability = "(suppressed)";
		let ability = toID(
			serverPokemon.ability || pokemon.ability || serverPokemon.baseAbility
		);
		...
		let [moveType, category] = this.getMoveType(
			move,
			value,
			gmaxMove || isZOrMax === "maxmove"
		);
		...
		if (isZOrMax === "zmove") {
			if (item.zMoveFrom === move.name) {
				...
				let moveName =
					BattleTooltips.zMoveTable[item.zMoveType as TypeName];
				...
				if (!movePower && move.id.startsWith("hiddenpower")) {
					movePower =
						this.battle.dex.moves.get("hiddenpower").zMove!.basePower;
				}
		...
		if (move.id === "naturepower") {
			...
			text +=
				"Calls " +
				Dex.getTypeIcon(this.getMoveType(calledMove, value)[0]) +
				" " +
				calledMove.name;
		}
		...
		if (this.battle.hardcoreMode) {
			...
			if (move.priority > 1) {
				text +=
					"Nearly always moves first <em>(priority +" +
					move.priority +
					")</em>.</p><p>";
			} else if (move.priority <= -1) {
				text +=
					"Nearly always moves last <em>(priority &minus;" +
					-move.priority +
					")</em>.</p><p>";
			} else if (move.priority === 1) {
				text +=
					"Usually moves first <em>(priority +" +
					move.priority +
					")</em>.</p><p>";
			} else {
				if (
			...
			if (
				...
				if (move.target === "allAdjacent") {
					...
					text +=
						"<p>&#x25ce; Can target distant Pok&eacute;mon in Triples.</p>";
				}
			...
			if (
				!move.flags.protect &&
				!["self", "allySide"].includes(move.target)
			) {
	...
	showPokemonTooltip(
		...
		if (gender === "M" || gender === "F") {
			genderBuf = ` <img src="${
				Dex.fxPrefix
			}gender-${gender.toLowerCase()}.png" alt="${gender}" width="7" height="10" class="pixelated" /> `;
		}
		...
		if (pokemon.speciesForme !== pokemon.name) {
			name +=
				" <small>(" +
				BattleLog.escapeHTML(pokemon.speciesForme) +
				")</small>";
		}
		...
		if (!illusionIndex || illusionIndex === 1) {
			text += `<h2>${name}${genderBuf}${
				illusionIndex ? "" : levelBuf
			}<br />`;
			...
			let types = serverPokemon?.terastallized
			...
			if (pokemon.terastallized) {
				...
				clientPokemon?.volatiles.typechange ||
				clientPokemon?.volatiles.typeadd
			) {
				text += `<small>(Type changed)</small><br />`;
			}
			text += `<span class="textaligned-typeicons">${types
				.map((type) => Dex.getTypeIcon(type))
				.join(" ")}</span>`;
			if (pokemon.terastallized) {
				text += `&nbsp; &nbsp; <small>(base: <span class="textaligned-typeicons">${this.getPokemonTypes(
					pokemon,
					true
				)
					.map((type) => Dex.getTypeIcon(type))
				...
				text += `&nbsp; &nbsp; <small>(Tera Type: <span class="textaligned-typeicons">${Dex.getTypeIcon(
					serverPokemon.teraType
				)}</span>)</small>`;
			}
			text += `</h2>`;
				...
				if (pokemon.status === "tox") {
					if (
						[
							...
							clientPokemon.getActiveAbilities(clientPokemon).includes(a)
						)
					) {
						text +=
							" <small>Would take if ability removed: " +
							Math.floor(
								(100 / 16) *
									Math.min(clientPokemon.statusData.toxicTurns + 1, 15)
							) +
							"%</small>";
					} else {
						text +=
							" Next damage: " +
							Math.floor(
								(100 / (clientPokemon.volatiles["dynamax"] ? 32 : 16)) *
									Math.min(clientPokemon.statusData.toxicTurns + 1, 15)
							) +
							"%";
					}
		...
		const supportsAbilities =
			this.battle.gen > 2 && !this.battle.tier.includes("Let's Go");
		...
		if (serverPokemon) {
			...
			if (clientPokemon?.prevItem) {
				...
				itemEffect += clientPokemon.prevItemEffect
					? prevItem + " was " + clientPokemon.prevItemEffect
					: "was " + prevItem;
			}
	...
	showFieldTooltip() {
		...
		for (const side of this.battle.sides) {
			...
			buf += `<td><p class="section"><strong>${BattleLog.escapeHTML(
				side.name
			)}</strong>${sideConditions || "<br />(no conditions)"}</p></td>`;
		}
		buf += `</tr><table>`;
		if (!atLeastOne) buf = ``;
	...
	calculateModifiedStats(
		...
		for (const statName of Dex.statNamesExceptHP) {
			...
			const clientStatName =
				clientPokemon.boosts.spc &&
				(statName === "spa" || statName === "spd")
					? "spc"
					: statName;
			...
			if (boostLevel) {
				...
				if (boostLevel > 0) {
					...
					if (this.battle.gen <= 2)
						boostTable = [1, 100 / 66, 2, 2.5, 100 / 33, 100 / 28, 4];
					stats[statName] /= boostTable[-boostLevel];
				}
		...
		const ability =
			clientPokemon?.effectiveAbility(serverPokemon) ??
			(serverPokemon.ability || serverPokemon.baseAbility);

		const activeAbilities =
			this.getPokemonInnateData(clientPokemon, serverPokemon).innates || [];
		if (ability) activeAbilities.push(ability);

		// check for burn, paralysis, guts, quick feet
		if (pokemon.status) {
			if (this.battle.gen > 2 && activeAbilities.includes("Guts")) {
				stats.atk = Math.floor(stats.atk * 1.5);
			} else if (this.battle.gen < 2 && pokemon.status === "brn") {
				stats.atk = Math.floor(stats.atk * 0.5);
			}

			if (this.battle.gen > 2 && activeAbilities.includes("Quick Feet")) {
		...
		if (
			(activeAbilities.includes("Klutz") &&
				!speedHalvingEVItems.includes(item)) ||
		...
		if (
			speciesName === "Ditto" &&
			!(clientPokemon && "transform" in clientPokemon.volatiles)
		) {
			if (item === "quickpowder") {
		...
		if (
			activeAbilities.includes("Pure Power") ||
			activeAbilities.includes("Huge Power")
		) {
			stats.atk *= 2;
		}
		if (
			activeAbilities.includes("Hustle") ||
			(activeAbilities.includes("Gorilla Tactics") &&
				!clientPokemon?.volatiles["dynamax"])
		) {
			stats.atk = Math.floor(stats.atk * 1.5);
		}
		if (
			activeAbilities.includes("Sage Power") &&
			!clientPokemon?.volatiles["dynamax"]
		) {
		...
		if (weather) {
			if (
				this.battle.gen >= 4 &&
				this.pokemonHasType(pokemon, "Rock") &&
				weather === "sandstorm"
			) {
				if (this.battle.weatherSource === "Ability") {
					//Sandstorm nerf
			...
			if (activeAbilities.includes("Sand Rush") && weather === "sandstorm") {
				speedModifiers.push(1.5);
			}
			if (
				activeAbilities.includes("Slush Rush") &&
			...
			if (item !== "utilityumbrella") {
				if (weather === "sunnyday" || weather === "desolateland") {
					if (activeAbilities.includes("Chlorophyll")) {
						speedModifiers.push(1.5);
					}
					if (activeAbilities.includes("Solar Power")) {
						stats.spa = Math.floor(stats.spa * 1.5);
					}
					if (activeAbilities.includes("Orichalcum Pulse")) {
						stats.atk = Math.floor(stats.atk * 1.3);
					}
					if (activeAbilities.includes("Big Leaves")) {
						stats.spa = Math.floor(stats.spa * 1.5);
						speedModifiers.push(1.5);
					}
					...
					if (allyActive) {
						for (const ally of allyActive) {
							...
							let allyAbilities = this.getAllyAbility(ally);
							if (
								allyAbilities.includes("Flower Gift") &&
								(ally.getSpecies().baseSpecies === "Cherrim" ||
									this.battle.gen <= 4)
							) {
								stats.atk = Math.floor(stats.atk * 1.5);
								stats.spd = Math.floor(stats.spd * 1.5);
				...
				if (weather === "raindance" || weather === "primordialsea") {
					if (activeAbilities.includes("Swift Swim")) {
						speedModifiers.push(1.5);
					}
				}
			}
		...
		if (
			activeAbilities.includes("Defeatist") &&
			serverPokemon.hp <= serverPokemon.maxhp / 2
		) {
			stats.atk = Math.floor(stats.atk * 0.5);
			stats.spa = Math.floor(stats.spa * 0.5);
		}
		if (clientPokemon) {
			if (clientPokemon.volatiles["Slow Start"]) {
				stats.atk = Math.floor(stats.atk * 0.5);
				speedModifiers.push(0.5);
			}
			if (
				activeAbilities.includes("Unburden") &&
				clientPokemon.volatiles["itemremoved"] &&
				!item
			) {
				speedModifiers.push(2);
			}
			for (const statName of Dex.statNamesExceptHP) {
				if (
					clientPokemon.volatiles["protosynthesis" + statName] ||
					clientPokemon.volatiles["quarkdrive" + statName]
				) {
					if (statName === "spe") {
						speedModifiers.push(1.5);
					} else {
		...
		if (activeAbilities.includes("Marvel Scale") && pokemon.status) {
			stats.def = Math.floor(stats.def * 1.5);
		}
		const isNFE = this.battle.dex.species
			.get(serverPokemon.speciesForme)
			.evos?.some((evo) => {
				const evoSpecies = this.battle.dex.species.get(evo);
				return (
					!evoSpecies.isNonstandard ||
					evoSpecies.isNonstandard ===
						this.battle.dex.species.get(serverPokemon.speciesForme)
							?.isNonstandard ||
		...
		if (
			activeAbilities.includes("Grass Pelt") &&
			this.battle.hasPseudoWeather("Grassy Terrain")
		) {
			stats.def = Math.floor(stats.def * 1.5);
		}
		if (this.battle.hasPseudoWeather("Electric Terrain")) {
			if (activeAbilities.includes("Surge Surfer")) {
				speedModifiers.push(1.5);
			}
			if (activeAbilities.includes("Hardon Engine")) {
		...
		if (
			clientPokemon &&
			["Plus", "Minus"].some((a) => activeAbilities.includes(a))
		) {
			let allyActive = clientPokemon.side.active;
			if (allyActive.length > 1) {
				//Plus and Minus should never be on same pokemon, so only check for one
				let abilityName = activeAbilities.includes("Plus")
					? "Plus"
					: "Minus";
				for (const ally of allyActive) {
					...
					let allyAbilities = this.getAllyAbility(ally);
					if (
						allyAbilities.includes("Plus") &&
						allyAbilities.includes("Minus")
					)
						continue;
					if (this.battle.gen <= 4 && allyAbilities.includes(abilityName))
						continue;
					stats.spa = Math.floor(stats.spa * 1.5);
					break;
				}
		...
		console.log(
			`clientPokemon: ${clientPokemon}, hasVR: ${activeAbilities.includes(
				"Violent Rush"
			)}, Active turns: ${clientPokemon?.lastMove}`
		);
		console.table(activeAbilities);
		if (
			clientPokemon &&
			activeAbilities.includes("Violent Rush") &&
			!clientPokemon.lastMove
		) {
		...
		if (activeAbilities.includes("Fur Coat")) {
			stats.def *= 2;
		}
		...
		stats.spe =
			stats.spe % 1 > 0.5 ? Math.ceil(stats.spe) : Math.floor(stats.spe);
	...
	renderStats(
		...
		if (!serverPokemon || isTransformed) {
			if (!clientPokemon)
				throw new Error("Must pass either clientPokemon or serverPokemon");
			...
			return (
				...
				" to " +
				max +
				" <small>(before items/abilities/modifiers)</small></p>"
			);
		}
		const stats = serverPokemon.stats;
		const modifiedStats = this.calculateModifiedStats(
			clientPokemon,
			serverPokemon
		);
		...
		if (!short) {
			...
			for (const statName of Dex.statNamesExceptHP) {
				...
				let statLabel =
					this.battle.gen === 1 && statName === "spa" ? "spc" : statName;
		...
		for (const statName of Dex.statNamesExceptHP) {
			...
			if (modifiedStats[statName] === stats[statName]) {
				...
				buf +=
					'<strong class="stat-lowered">' +
					modifiedStats[statName] +
					"</strong>";
			} else {
				buf +=
					'<strong class="stat-boosted">' +
					modifiedStats[statName] +
					"</strong>";
			}
		}
	...
	getPPUseText(moveTrackRow: [string, number], showKnown?: boolean) {
		...
		if (ppUsed || moveName.charAt(0) === "*") {
			return `${bullet} ${move.name} <small>(${
				maxpp - ppUsed
			}/${maxpp})</small>`;
		}
		return `${bullet} ${move.name} ${
			showKnown ? " <small>(revealed)</small>" : ""
		}`;
	}
	...
	getSpeedRange(pokemon: Pokemon): [number, number] {
		...
		if (tier.includes("Let's Go")) {
			...
			max = tr(
				tr(((2 * baseSpe + maxIvEvOffset) * level) / 100 + 5) * maxNature
			);
			min = isCGT
				? max
				: tr(tr((2 * baseSpe * level) / 100 + 5) * minNature);
	...
	getMoveType(
		...
		forMaxMove?: boolean | Move
	): [TypeName, "Physical" | "Special" | "Status"] {
		...
		if (
			move.id === "aurawheel" &&
			pokemon.getSpeciesForme() === "Morpeko-Hangry"
		) {
			moveType = "Dark";
		}
		...
		if (allowTypeOverride) {
			if (this.battle.rules["Revelationmons Mod"]) {
				...
				for (let i = 0; i < types.length; i++) {
					if (
						serverPokemon.moves[i] &&
						move.id === toID(serverPokemon.moves[i])
					) {
						moveType = types[i];
					}
				}
			...
			if (
				...
				if (moveType === "Normal") {
					...
					if (value.abilityModify(0, "Crystallize")) moveType = "Rock";
					if (value.abilityModify(0, "Refrigerate")) moveType = "Ice";
					if (value.abilityModify(0, "Burnate")) moveType = "Fire";
					if (value.abilityModify(0, "Solar Flare")) moveType = "Fire";
					if (value.abilityModify(0, "Fighting Spirit"))
						moveType = "Fighting";
			...
			const isSound = !!(
				forMaxMove
					? this.getMaxMoveFromType(
							moveType,
							(forMaxMove !== true && forMaxMove) || undefined
					  )
					: move
			).flags["sound"];
		...
		if (
			...
			const stats = this.calculateModifiedStats(
				pokemon,
				serverPokemon,
				true
			);
	...
	getMoveAccuracy(move: Move, value: ModifiableValue, target?: Pokemon) {
		...
		if (
			move.id === "toxic" &&
			this.battle.gen >= 6 &&
			this.pokemonHasType(pokemon, "Poison")
		) {
		...
		if (move.ohko) {
			...
			if (
				move.id === "sheercold" &&
				this.battle.gen >= 7 &&
				!this.pokemonHasType(pokemon, "Ice")
			) {
			...
			if (target) {
				if (pokemon.level < target.level) {
					...
					value.set(
						value.value + pokemon.level - target.level,
						"+1% per level above target"
					);
				}
			} else {
				if (pokemon.level < 100)
					value.set(value.value, "fails if target's level is higher");
				if (pokemon.level > 1)
					value.set(value.value, "+1% per level above target");
			}
			return value;
		}
		...
		for (const active of pokemon.side.active) {
			...
			const abilities = this.getAllyAbility(active);
			if (abilities.includes("Victory Star")) {
		...
		accuracyAfterChain =
			accuracyAfterChain % 1 > 0.5
				? Math.ceil(accuracyAfterChain)
				: Math.floor(accuracyAfterChain);
		...
		if (pokemon?.boosts.accuracy) {
			if (pokemon.boosts.accuracy > 0) {
				value.set(
					Math.floor((value.value * (pokemon.boosts.accuracy + 3)) / 3)
				);
			} else {
				value.set(
					Math.floor((value.value * 3) / (3 - pokemon.boosts.accuracy))
				);
			}
		}
		...
		if (
			this.battle.gen === 1 &&
			!toID(this.battle.tier).includes("stadium")
		) {
	...
	getMoveBasePower(
		...
		const activeAbilities = value.innates;
		...
		if (
			//We can keep Battle Bond's ability check here because Battle Bond should never be an innate
			move.id === "watershuriken" &&
			pokemon.getSpeciesForme() === "Greninja-Ash" &&
			pokemon.ability === "Battle Bond"
		) {
		...
		if (
			["lowkick", "grassknot", "heavyslam", "heatcrash"].includes(move.id)
		) {
		...
		if (move.id === "ragefist") {
			value.set(
				...
				pokemon.timesAttacked > 0
					? `Hit ${pokemon.timesAttacked} time${
							pokemon.timesAttacked > 1 ? "s" : ""
					  }`
					: undefined
			);
		}
		...
		if (move.flags["punch"]) {
			...
			value.abilityModify(1.3, "Power Fists");
		}
		if (move.flags["pulse"]) {
		...
		if (
			["Rock", "Ground", "Steel"].includes(moveType) &&
			this.battle.weather === "sandstorm"
		) {
			if (value.tryAbility("Sand Force"))
				value.weatherModify(1.3, "Sandstorm", "Sand Force");
		}
		if (move.secondaries) {
		...
		if (move.flags["contact"]) {
			...
			value.abilityModify(1.3, "Big Pecks");
		}
		if (!move.flags["contact"]) {
			value.abilityModify(1.2, "Long Reach");
		}
		if (move.flags["sound"]) {
			value.abilityModify(1.3, "Punk Rock");
			value.abilityModify(1.2, "Amplifier");
		}
		if (move.flags["slicing"]) {
			value.abilityModify(1.5, "Sharpness");
			value.abilityModify(1.3, "Keen Edge");
		}
		if (move.flags["kick"]) {
		...
		//TODO: Would likely be easier and slightly faster to combine lists and only loop once
		if (move.category !== "Status") {
			for (const ally of pokemon.side.active) {
				if (!ally || ally.fainted) continue;
				if (
					ally.status === "slp" ||
					ally.getActiveAbilities(ally).includes("Comatose")
				) {
					value.abilityModify(2, "Dreamcatcher");
					break;
				}
			}
			for (const foe of pokemon.side.foe.active) {
				...
				if (
					foe.status === "slp" ||
					foe.getActiveAbilities(foe).includes("Comatose")
				) {
					value.abilityModify(2, "Dreamcatcher");
					break;
				}
		...
		if (move.category !== "Status") {
			...
			for (const ally of pokemon.side.active) {
				if (!ally || ally.fainted) continue;
				let allyAbilities = this.getAllyAbility(ally);
				if (moveType === "Fairy" && allyAbilities.includes("Fairy Aura")) {
					auraBoosted = "Fairy Aura";
				} else if (
					moveType === "Dark" &&
					allyAbilities.includes("Dark Aura")
				) {
					auraBoosted = "Dark Aura";
				} else if (allyAbilities.includes("Aura Break")) {
					auraBroken = true;
				} else if (
					allyAbilities.includes("Battery") &&
					ally !== pokemon &&
					move.category === "Special"
				) {
					value.modify(1.3, "Battery");
				} else if (
					allyAbilities.includes("Power Spot") &&
					ally !== pokemon
				) {
					value.modify(1.3, "Power Spot");
				} else if (
					allyAbilities.includes("Steely Spirit") &&
					moveType === "Steel"
				) {
			...
			for (const foe of pokemon.side.foe.active) {
				if (!foe || foe.fainted) continue;
				if (
					foe.getActiveAbilities(foe).includes("Fairy Aura") &&
					moveType === "Fairy"
				) {
					auraBoosted = "Fairy Aura";
				} else if (
					foe.getActiveAbilities(foe).includes("Dark Aura") &&
					moveType === "Dark"
				) {
					auraBoosted = "Dark Aura";
				} else if (foe.getActiveAbilities(foe).includes("Aura Break")) {
					auraBroken = true;
				}
			}
			if (auraBoosted) {
	...
	getItemBoost(move: Move, value: ModifiableValue, moveType: TypeName) {
		...
		if (
			BattleTooltips.orbUsers[speciesName]?.includes(item.name) &&
	...
	getAllyAbility(ally: Pokemon) {
		...
		if (!allyAbility) {
			if (this.battle.myAllyPokemon) {
				// multi battle ally
				allyAbility = Dex.abilities.get(
					this.battle.myAllyPokemon[ally.slot].ability
				).name;
			} else if (this.battle.myPokemon) {
				allyAbility = Dex.abilities.get(
					this.battle.myPokemon[ally.slot].ability
				).name;
			}
		}
		if (!this.battle.tier.includes("Elite Redux")) return allyAbility;
		const innateList = [];
		if (ally) {
			const speciesForme =
				ally.getSpeciesForme() || ally?.speciesForme || "";
			const species = this.battle.dex.species.get(speciesForme);
			if (species.exists && species.abilities["I1"]) {
				if (species.abilities["I1"])
					innateList.push(species.abilities["I1"]);
	...
	getPokemonAbilityData(
		...
		const innateData = this.getPokemonInnateData(
			clientPokemon,
			serverPokemon
		).innates;
		if (clientPokemon) {
			if (
				clientPokemon.ability &&
				!innateData.includes(clientPokemon.ability)
			) {
				if (!innateData.includes(abilityData.ability))
					abilityData.ability =
						clientPokemon.ability || clientPokemon.baseAbility;
				if (clientPokemon.baseAbility) {
					abilityData.baseAbility = clientPokemon.baseAbility;
				}
			} else {
				const speciesForme =
					clientPokemon.getSpeciesForme() ||
	...
	getPokemonInnateData(
		...
		if (clientPokemon) {
			...
			if (species.exists && species.abilities["I1"]) {
				if (species.abilities["I1"])
					innateData.innates.push(species.abilities["I1"]);
				if (species.abilities["I2"])
					innateData.innates.push(species.abilities["I2"]);
				if (species.abilities["I3"])
					innateData.innates.push(species.abilities["I3"]);
			}
		}
		return innateData;
	}
	...
	getPokemonAbilityText(
		...
		let text = "";
		const abilityData = this.getPokemonAbilityData(
			clientPokemon,
			serverPokemon
		);
		if (!isActive) {
			...
			if (ability)
				text = "<small>Ability:</small> " + Dex.abilities.get(ability).name;
		} else {
			if (abilityData.ability) {
				const abilityName = Dex.abilities.get(abilityData.ability).name;
				text = "<small>Ability:</small> " + abilityName;
				const baseAbilityName = Dex.abilities.get(
					abilityData.baseAbility
				).name;
				//checks if new ability is an innate or not
		...
		if (!text && abilityData.possibilities.length && !hidePossible) {
			text =
				...
				abilityData.possibilities.join(", ");
		}
		return text;
	}
...
class BattleStatGuesser {
	...
	constructor(formatid: ID) {
		...
		} else if (formatid.startsWith("eliteredux")) {
			console.debug("found elite redux modid", formatid);
			this.formatid = "eliteredux" as ID;
			this.dex = Dex.mod("eliteredux" as ID);
		} else {
	...
	guessRole(set: PokemonSet) {
		...
		for (let i = 0, len = set.moves.length; i < len; i++) {
			...
			if (move.category === "Status") {
				if (["batonpass", "healingwish", "lunardance"].includes(move.id)) {
					...
					["protect", "detect", "spikyshield", "kingsshield"].includes(
						move.id
					)
				) {
					moveCount["Stall"]++;
				...
				if (
					["scald", "voltswitch", "uturn", "flipturn"].includes(move.id)
				) {
		...
		if (hasMove["dragondance"] || hasMove["quiverdance"])
		...
		if (
			((hasMove["bodyslam"] || hasMove["thunder"]) &&
				abilityid === "serenegrace") ||
			hasMove["thunderwave"]
		) {
			physicalBulk *= 1.1;
			specialBulk *= 1.1;
		...
		if (
			(hasMove["ironhead"] || hasMove["airslash"]) &&
			abilityid === "serenegrace"
		) {
			physicalBulk *= 1.1;
			specialBulk *= 1.1;
		...
		if (
			(itemid === "flameorb" || itemid === "toxicorb") &&
			abilityid !== "magicguard"
		) {
		...
		if (moveCount["PhysicalStall"] && moveCount["Restoration"]) {
			if (stats.spe > 110 && abilityid !== "prankster")
		...
		if (
			...
			if (isFast) {
				if (bulk > 80000 && !moveCount["Ultrafast"])
		...
		if (species.id === "blissey" || species.id === "chansey")
			return "Physically Defensive";
	...
	guessEVs(
		...
		if (this.supportsAVs) {
			...
			if (secondaryStat === "hp" && set.level && set.level < 20)
				secondaryStat = "spd";
			stat = this.getStat(
				secondaryStat,
				set,
			...
			if (hpDivisibility) {
				while (
					hp < 252 &&
					evTotal < 508 &&
					!(stat % hpDivisibility) !== hpShouldBeDivisible
				) {
					hp += 4;
					stat = this.getStat("hp", set, hp, 1);
					evTotal += 4;
