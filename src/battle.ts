...
export class Pokemon implements PokemonDetails, PokemonHealth {
	...
	healthParse(
		...
		if (parenIndex >= 0) {
			...
			hpstring = hpstring.substr(
				parenIndex + 1,
				hpstring.length - parenIndex - 2
			);
		}
	...
	getBoost(boostStat: BoostStatName) {
		...
		if (this.boosts[boostStat] < -6) this.boosts[boostStat] = -6;
		const isRBY =
			this.side.battle.gen <= 1 &&
			!this.side.battle.tier.includes("Stadium");
		if (!isRBY && (boostStat === "accuracy" || boostStat === "evasion")) {
			if (this.boosts[boostStat] > 0) {
				...
				return (
					"" +
					goodBoostTable[this.boosts[boostStat]] +
					"&nbsp;" +
					boostStatTable[boostStat]
				);
			}
			let badBoostTable = [
				"1&times;",
			...
			return (
				"" +
				badBoostTable[-this.boosts[boostStat]] +
				"&nbsp;" +
				boostStatTable[boostStat]
			);
		}
	...
	getWeightKg(serverPokemon?: ServerPokemon) {
		...
		return Math.max(
			this.getSpecies(serverPokemon).weightkg - autotomizeFactor,
			0.1
		);
	}
	...
	): [ReadonlyArray<TypeName>, TypeName | ""] {
	...
	isGrounded(serverPokemon?: ServerPokemon) {
		...
		if (
			battle.hasPseudoWeather("Magic Room") ||
	...
	getSpecies(serverPokemon?: ServerPokemon) {
		return this.side.battle.dex.species.get(
			this.getSpeciesForme(serverPokemon)
		);
	}
	getBaseSpecies() {
...
export class Side {
	...
	addPokemon(name: string, ident: string, details: string, replaceSlot = -1) {
		...
		if (
			this.pokemon.length > this.totalPokemon ||
			this.battle.speciesClause
		) {
			// check for Illusion
			...
			if (toRemove >= 0) {
				if (this.pokemon[toRemove].fainted) {
					...
					for (const curPoke of this.pokemon) {
						...
						if (
							curPoke.speciesForme === "Zoroark" ||
							curPoke.speciesForme === "Zorua" ||
	...
	switchIn(pokemon: Pokemon, kwArgs: KWArgs, slot = pokemon.slot) {
		...
		if (["batonpass", "zbatonpass", "shedtail"].includes(effect.id)) {
			pokemon.copyVolatileFrom(
				this.lastPokemon!,
				effect.id === "shedtail" ? "shedtail" : false
			);
		}
	...
	switchOut(pokemon: Pokemon, kwArgs: KWArgs, slot = pokemon.slot) {
		...
		if (
			!["batonpass", "zbatonpass", "shedtail", "teleport"].includes(
				effect.id
			)
		) {
...
export class Battle {
	...
	constructor(
		...
		if (options.$frame && options.$logFrame) {
			...
			throw new Error(
				`You must specify $frame and $logFrame simultaneously`
			);
		}
	...
	abilityActive(abilities: string | string[]) {
		...
		if (this.ngasActive()) {
			abilities = abilities.filter(
				(a) => this.dex.abilities.get(a).isPermanent
			);
			...
			if (this.abilityActive("Pressure") && move.id !== "stickyweb") {
				...
				if (
					...
					for (const active of this.getAllActive()) {
						...
						if (
							this.gen <= 4 ||
							(active.side !== pokemon.side &&
								active.side.ally !== pokemon.side)
						) {
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		if (nextArgs && nextKwargs) {
			...
			if (
				["-crit", "-supereffective", "-resisted"].includes(args[0]) ||
				args[2] === "ability: Sturdy"
			) {
				kwArgs.then = ".";
			}
			...
			if (
				...
				nextArgs[0] === "-damage" &&
				kwArgs.from &&
				kwArgs.from === nextKwargs.from
			) {
				kwArgs.then = ".";
			}
			if (
				args[0] === "-ability" &&
				(args[2] === "Intimidate" || args[3] === "boost")
			) {
				kwArgs.then = ".";
			}
		...
		switch (args[0]) {
			case "-damage": {
				...
				if (kwArgs.from) {
					...
					if (effect.effectType === "Item") {
						...
						if (
							itemPoke.prevItem !== effect.name &&
							!CONSUMED.includes(itemPoke.prevItemEffect)
						) {
			...
			case "-heal": {
				...
				if (kwArgs.from) {
					...
					if (
						...
						!CONSUMED.includes(poke.prevItemEffect)
					) {
			...
			case "-sethp": {
				for (let k = 0; k < 2; k++) {
					...
					if (cpoke) {
						...
						let formattedRange = Pokemon.getFormattedRange(
							range,
							0,
							" to "
						);
				...
				if (effect.id) {
					switch (effect.id) {
						case "spectralthief":
							...
							this.scene.runOtherAnim("spectralthiefboost" as ID, [
								ofpoke!,
								poke,
							]);
							break;
			...
			case "-immune": {
				...
				this.activateAbility(
					this.getPokemon(kwArgs.of) || poke,
					fromeffect
				);
				this.log(args, kwArgs);
			...
			case "-prepare": {
				...
				let target =
					this.getPokemon(args[3]) || poke.side.foe.active[0] || poke;
			...
			case "detailschange": {
				...
				poke.ability = poke.baseAbility = species.abilities
					? species.abilities["0"]
					: "";
			...
			case "-transform": {
				...
				const speciesForme =
					targetForme && !targetForme[1].endsWith("-Gmax")
						? targetForme[1]
						: tpoke.speciesForme;
				const pokemon = tpoke;
			...
			case "-formechange": {
				...
				if (
					!poke.getSpeciesForme().endsWith("-Gmax") &&
					!species.name.endsWith("-Gmax")
				) {
					poke.removeVolatile("typeadd" as ID);
					poke.removeVolatile("typechange" as ID);
						...
						default:
							if (effect.effectType === "Move") {
								if (effect.name === "Doom Desire") {
									this.scene.runOtherAnim("doomdesirehit" as ID, [
										poke,
									]);
								}
								if (effect.name === "Future Sight") {
									this.scene.runOtherAnim("futuresighthit" as ID, [
										poke,
									]);
								}
							}
			...
			case "-singleturn": {
				...
				if (
					effect.id === "roost" &&
					!poke.getTypeList().includes("Flying")
				) {
					break;
				}
			...
			case "-activate": {
				...
				switch (effect.id) {
					...
					case "wanderingspirit":
						if (this.gen <= 4) break;
						let pokeability =
							Dex.sanitizeName(kwArgs.ability) || target!.ability;
						let targetability =
							Dex.sanitizeName(kwArgs.ability2) || poke.ability;
						if (pokeability) {
	...
	*/
	...
	parseDetails(
		...
		if (
			splitDetails[splitDetails.length - 1] === "M" ||
			splitDetails[splitDetails.length - 1] === "F"
		) {
	...
	parsePokemonId(pokemonid: string) {
		...
		if (/^p[1-9]($|: )/.test(name)) {
			...
			const slotChart: { [k: string]: number } = {
				a: 0,
				b: 1,
				c: 2,
				d: 3,
	...
	run(str: string, preempt?: boolean) {
		if (
			!preempt &&
			this.preemptStepQueue.length &&
			str === this.preemptStepQueue[0]
		) {
		...
		if (nextLine.slice(0, 2) === "|-") {
			({ args: nextArgs, kwArgs: nextKwargs } =
				BattleTextParser.parseBattleLine(nextLine));
		}
